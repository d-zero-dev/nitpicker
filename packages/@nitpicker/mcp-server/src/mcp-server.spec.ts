...
describe('createServer', () => {
	...
	it('ListTools で34個のツールが返される', async () => {
		...
		expect(names).toContain('list_pages_by_technology');
		expect(names).toContain('list_pages_by_jsonld_type');
		expect(names).toContain('get_technology_inventory');
		expect(names).toContain('get_page_jsonld');
		expect(names).toContain('get_page_technologies');
		...
		expect(names).toContain('count_pages_by_technology');
