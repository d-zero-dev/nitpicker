...
describe('registerPagesRoute (integration)', () => {
	...
	describe('urlPattern stays on the fast path (keyset cursor, redirect/alias parity)', () => {
		...
		it('mints an opaque keyset cursor, not the live plain-decimal offset cursor (regression: forcing live would change the cursor format)', async () => {
			const res = await fixture.app.request(
				`/api/pages?limit=2&urlPattern=${encodeURIComponent('%example.com%')}`,
			);
	...
	describe('hasCSP filter stays on the fast path (dedicated viewer_pages column, no live fallback)', () => {
		const workingDir = path.resolve(
			__dirname,
	...
	describe('lang filter on the fast path (regression: silently ignored, always returning every page instead of only the matching lang)', () => {
		const workingDir = path.resolve(
			__dirname,
			'__test_fixtures_register_pages_route_lang_filter__',
		);
		let fixture: { app: ReturnType<typeof createApp>; manager: ArchiveManager };
		...
		beforeAll(async () => {
			...
			await archive.setConfig(BASE_CONFIG);
			await archive.setPage({
				url: parseUrl('https://example.com/ja')!,
				redirectPaths: [],
				isExternal: false,
				isTarget: true,
				status: 200,
			...
			await archive.setPage({
				url: parseUrl('https://example.com/en')!,
				redirectPaths: [],
				isExternal: false,
				isTarget: true,
				status: 200,
			...
			const app = createApp({
				...
				publicDir: '/tmp/no-such-dir-register-pages-route-lang-spec',
			});
			fixture = { app, manager };
		});
		...
		it('returns only the ja page, not both pages regardless of lang', async () => {
			const res = await fixture.app.request('/api/pages?lang=ja');
			const body = (await res.json()) as { items: { url: string }[]; total: number };
	...
	describe('stale read model on a finished archive (not stub) refuses /api/pages instead of silently degrading to the live query', () => {
		const workingDir = path.resolve(
			__dirname,
			'__test_fixtures_register_pages_route_stale_read_model__',
		);
		...
		beforeAll(async () => {
			...
			app = createApp({
				context: {
					...
					archiveId: 'test-pages-stale',
					filePath: workingDir,
					mode: 'archive',
					crawlerLockHolder: null,
				},
				publicDir: '/tmp/no-such-dir-register-pages-route-stale-spec',
			});
		});
