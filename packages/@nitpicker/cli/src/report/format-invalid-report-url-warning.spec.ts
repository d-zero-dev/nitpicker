...
import { formatInvalidReportUrlWarning } from './format-invalid-report-url-warning.js';

describe('formatInvalidReportUrlWarning', () => {
	it('リストファイル名・行・列・不正な値を含むメッセージを組み立てる', () => {
		const message = formatInvalidReportUrlWarning('urls.txt', {
			value: 'not-a-url',
			line: 3,
			column: 1,
		});
	...
	it('解決済みの絶対パスではなく、オペレータが入力した文字列をそのまま使う', () => {
		const message = formatInvalidReportUrlWarning('./relative/urls.txt', {
			value: 'bad',
			line: 1,
			column: 1,
		});
