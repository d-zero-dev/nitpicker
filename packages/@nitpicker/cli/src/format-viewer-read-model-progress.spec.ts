...
describe('formatViewerReadModelProgress', () => {
	...
	it('labels computingSummary progress with a "steps" unit (issue #294)', () => {
		...
		).toBe('Computing summary: 1/3 steps (33%)');
	});
	...
	it('labels buildingGraph progress with an "edge ids" unit (issue #294)', () => {
		...
		).toBe('Building link graph: 2,000/10,000 edge ids (20%)');
	});
	...
	it('labels buildingMismatches progress with a "scans" unit (issue #294)', () => {
		...
		).toBe('Building mismatches: 3/6 scans (50%)');
	});

	it('renders no animation placeholder (issue #294) — the TaskList row icon animates instead', () => {
		expect(formatViewerReadModelProgress({ insertedRows: 1, totalRows: 2 })).not.toMatch(
			/%braille%|%dots%/,
		);
	});
});
