...
import { formatRecrawlSkipSummary } from './format-recrawl-skip-summary.js';

describe('formatRecrawlSkipSummary', () => {
	it('スキップ件数・総数・継続する有効件数を含むメッセージを組み立てる', () => {
		const message = formatRecrawlSkipSummary(12, 1234);
		expect(message).toBe(
	...
	it('スキップ件数が総数と一致する場合、継続件数は0になる', () => {
		const message = formatRecrawlSkipSummary(5, 5);
