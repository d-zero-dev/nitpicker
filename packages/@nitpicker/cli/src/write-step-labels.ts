 ...
 * by `viewer-build.ts` and `event-assignments.ts` (the crawl commands' write
 * progress) so the two can't drift into inconsistent wording for the same
 * step. `checkpoint` and `remove` have no countable progress of their own (a
