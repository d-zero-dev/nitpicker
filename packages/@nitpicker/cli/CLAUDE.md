...
- **crawl 本体の `Lanes` は CLI（`crawl.ts` の `prepareCrawlDisplay`）が生成して `Crawler` に注入する**（TTY・非 verbose・非 silent のときのみ）。crawler パッケージは受け取ったものを `deal()` に素通しするだけで、表示の判断（TTY 判定・verbose 判定・入力行の描画）は一切持たない。注入した `Lanes` は `deal()` が生成・破棄しなくなるため（`@d-zero/dealer` 1.14.0 の `DealOptions.lanes`）、auto-retry の待機ラウンドをまたいで生き続ける — `attachCrawlDisplay` の `onBeforeStart`（flush/sort `TaskList` が描画を始める直前、`startIfNeeded()` の中）が唯一の破棄ポイントで、`commands/crawl.ts` の `disposeCrawlDisplay` がここと `createOrchestratorFailingSetupOnError` の catch の両方から呼ばれる（後者は factory が `initializedCallback` 前に throw して `attachCrawlDisplay` 自体が一度も呼ばれないケースをカバーする）
  - **実行中のキー入力（`crawl/create-crawl-console.ts`）は注入した `Lanes` の `footer()` に描く** — 別途 `Lanes`/`Display` を作らない。TTY のときだけ stdin を raw mode にし、`parallels`/`interval`/`exclude`/`exclude-url`/`exclude-keyword`/`help` を受け付けて `CrawlerOrchestrator#updateRuntimeOptions` に橋渡しする（`crawl/parse-crawl-console-command.ts`）。Ctrl-C は raw mode で OS の SIGINT に変換されないため、`createCrawlConsole` の `onInterrupt` から `commands/crawl.ts` の `createCrawlInterruptAction` を直接呼ぶ（OS シグナルハンドラと同じアクション）
  - **シグナルハンドラは `Lanes` 構築より前に登録する**（`prepareCrawlDisplay`、各 crawl モード関数の先頭・`createSetupTaskList` より前）。`Lanes`/`Display` は非 verbose だと自前で SIGINT ハンドラを登録するため、こちらが後から登録されると Ctrl-C 時に `Display` の `process.exit(130)` が先に走り `orchestrator.abort()`/`garbageCollect()` が飛ぶ
- **`TaskList` と `Lanes`/`deal()` の同時稼働は禁止**（`Lanes`/`Display` の単一インスタンス制約、下記 ARCHITECTURE.md 参照）。`TaskList.run()` の pipe コールバック内で新しい `Lanes`/`deal()` を起動しない。crawl コマンドは `TaskList(起動ログ) → close → deal(crawl本体、注入した Lanes ごと) → close → TaskList(後処理)` の厳密な逐次で、区間が重ならないことを都度確認する
