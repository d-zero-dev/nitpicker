 ...
 *
 * A scope entry is a `(hostname, path)` pair. The target URL belongs to a scope
 * entry when its hostname matches and its path is at the same level or deeper
 * than the scope entry's path. Among all matching entries, the one with the
 * greatest depth wins (e.g. `/blog/2024/` is preferred over `/blog/`).
 *
