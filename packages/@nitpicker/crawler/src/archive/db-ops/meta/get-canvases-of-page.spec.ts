...
import { getCanvasesOfPage } from './get-canvases-of-page.js';

describe('getCanvasesOfPage', () => {
	let db: Knex;
	...
	it('returns rows in DOM traversal order', async () => {
		...
		const [result] = await getCanvasesOfPage(db, page.id);
		expect(result!.width).toBe(300);
		expect(result!.height).toBe(150);
	});

	it('returns an empty array when the page has no canvases', async () => {
		const [urlRef] = await db('url_refs')
			.insert({ url: 'https://example.com/' })
			.returning('id');
		const [page] = await db('content_items')
		...
		expect(await getCanvasesOfPage(db, page.id)).toEqual([]);
	});
});
