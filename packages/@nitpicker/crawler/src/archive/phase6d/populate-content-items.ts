...
/**
 * Populates `content_items` from `pages` (issue #193 step 6-D-1).
 *
 * Strategy per chunk:
 *
 ...
 *    Same pattern as {@link ../phase6b/populate-url-refs.ts}.
 ...
 * {@link ../create-phase6c-entity-tables.ts}) so a redirect source
...
export async function populateContentItems(trx: Knex): Promise<void> {
	...
	while (true) {
		...
		const inserts = rows.map((row) => {
			...
			if (urlId === null) {
				throw new Error(
					`populateContentItems: url_refs.id not resolved for page id=${row.id} url=${row.url} — Phase 6-B-1 populate must run first`,
				);
			}
			...
			if (row.contentType != null && row.contentType !== '') {
				...
				if (resolved === undefined) {
					throw new Error(
						`populateContentItems: content_type_refs.id not resolved for page id=${row.id} contentType=${row.contentType} — Phase 6-B-0 populate must run first`,
					);
				}
