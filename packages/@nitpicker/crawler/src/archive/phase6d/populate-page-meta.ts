 ...
 * the reader working set narrow and matches the Phase 6-E check
 ...
 *    FK on `content_items(id)`; Phase 6-D-1 must have already inserted
 ...
 * threshold — i.e. one that Phase 6-B-1 sent to `blob_refs` instead of
 ...
 * @param textIds - Map from Phase 6-B-2's populate step.
...
function resolveOptionalTextId(
	...
	if (id === undefined) {
		throw new Error(
			`populatePageMeta: text_refs.id not resolved for page id=${pageId} column=${column} — Phase 6-B-2 populate must run first`,
		);
	}
	return id;
}
 ...
 * warning) for large data URIs that Phase 6-B-1 routed to `blob_refs`
 * — those cannot land in `page_meta` because the schema has no
 * `*_blob_id` companion columns.
 * @param value - Raw column value from the source `pages` row.
 * @param urlIds - Map from Phase 6-B-1's populate step.
 * @param pageId - Owning page id (for the error message).
 * @param column - Source column name (for the error message).
 */
function resolveOptionalUrlId(
	...
	if (id === undefined) {
		throw new Error(
			`populatePageMeta: url_refs.id not resolved for page id=${pageId} column=${column} — Phase 6-B-1 populate must run first`,
		);
	}
	return id;
}
 ...
 * @param jsonIds - Map from Phase 6-B-3's populate step.
 * @param pageId - Owning page id (for the error message).
 */
function resolveOptionalJsonId(
	...
	if (id === undefined) {
		throw new Error(
			`populatePageMeta: json_refs.id not resolved for page id=${pageId} column=meta_extras — Phase 6-B-3 populate must run first`,
		);
	}
	return id;
}
