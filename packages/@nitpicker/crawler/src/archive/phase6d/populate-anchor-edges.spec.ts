...
/**
 * Inserts N pages so `anchor_edges` FKs resolve. `populatePhase6BRefs`
...
/**
 * Runs Phase 6-B populate + Phase 6-D content_items so anchor_edges'
 ...
 * @param db - Test DB instance.
 */
async function completePhase6Prerequisites(db: ReturnType<typeof knex>): Promise<void> {
...
describe('populateAnchorEdges (anchor-edge-normalization)', () => {
	...
	it('collapses duplicate anchors sharing (pageId, hrefId) with first-wins semantics', async () => {
		...
		await completePhase6Prerequisites(db);
