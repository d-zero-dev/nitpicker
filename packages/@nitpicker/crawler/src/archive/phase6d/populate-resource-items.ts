...
export async function populateResourceItems(trx: Knex): Promise<void> {
	...
	while (true) {
		...
		const inserts = rows.map((row) => {
			...
			if (urlId === null) {
				throw new Error(
					`populateResourceItems: url_refs.id not resolved for resource id=${row.id} url=${row.url} — Phase 6-B-1 populate must run first`,
				);
			}
			...
			if (row.contentType != null && row.contentType !== '') {
				...
				if (resolved === undefined) {
					throw new Error(
						`populateResourceItems: content_type_refs.id not resolved for resource id=${row.id} contentType=${row.contentType} — Phase 6-B-0 populate must run first`,
					);
				}
