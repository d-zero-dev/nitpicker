 ...
 *   `page_html_blobs`) — only the columns the Phase 6-D populate steps
 *   actually read.
 * - The Phase 6-A ref / header tables (via {@link createPhase6ARefTables}).
 * - The Phase 6-C entity tables (via {@link createPhase6CEntityTables}).
 *
 * Every Phase 6-D populate spec calls this to obtain a fresh DB. The
 ...
 */
export async function setupPhase6DDb(): Promise<ReturnType<typeof knex>> {
	const db = knex({
