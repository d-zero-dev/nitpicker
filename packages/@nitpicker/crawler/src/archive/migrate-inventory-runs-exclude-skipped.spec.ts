...
describe('migrateInventoryRunsExcludeSkipped', () => {
	it('adds the exclude_skipped column to an existing inventory_runs table', async () => {
		const { instance } = await buildLegacyInventoryRuns('migrate-exclude-test.sqlite');
		await instance('inventory_runs').insert({
			ran_at: '2026-01-01T00:00:00Z',
			scope_skipped: 3,
		});

		await migrateInventoryRunsExcludeSkipped(instance);

		expect(await instance.schema.hasColumn('inventory_runs', 'exclude_skipped')).toBe(
			true,
		);
	...
	it('is idempotent — calling twice on an up-to-date schema is a no-op', async () => {
		...
		await migrateInventoryRunsExcludeSkipped(instance);
		await expect(migrateInventoryRunsExcludeSkipped(instance)).resolves.toBeUndefined();
