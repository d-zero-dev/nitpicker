...
afterEach(async () => {
	for (const name of [
		'migrate-console-error-count-test.sqlite',
		'migrate-console-error-count-idempotent.sqlite',
		'migrate-console-error-count-empty.sqlite',
	]) {
		await fs.rm(path.resolve(workingDir, name), { force: true });
	}
});

describe('migratePageMetaConsoleErrorCount', () => {
	it('adds the console_error_count column to an existing page_meta', async () => {
		const { instance } = await buildLegacyPageMeta(
			'migrate-console-error-count-test.sqlite',
		);
		await instance('page_meta').insert({ page_id: 1, lang: 'ja' });

		await migratePageMetaConsoleErrorCount(instance);

		expect(await instance.schema.hasColumn('page_meta', 'console_error_count')).toBe(
			true,
		);

		// Pre-existing row and columns survive untouched, new column is NULL.
		const [row] = await instance.select('lang', 'console_error_count').from('page_meta');
		expect(row.lang).toBe('ja');
		expect(row.console_error_count).toBeNull();
	...
	it('is idempotent — calling twice on an up-to-date schema is a no-op', async () => {
		...
		await migratePageMetaConsoleErrorCount(instance);
		await expect(migratePageMetaConsoleErrorCount(instance)).resolves.toBeUndefined();
	...
	it('returns silently when page_meta does not exist', async () => {
		const filename = path.resolve(workingDir, 'migrate-console-error-count-empty.sqlite');
		await fs.rm(filename, { force: true });
		const instance = knex({
			client: LibsqlDialect as never,
			connection: { filename },
