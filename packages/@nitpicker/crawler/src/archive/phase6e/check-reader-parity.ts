...
/**
 * Compares totals reported by Phase 6-F readers (which query the Phase 6-C
 ...
 * Phase 6-D populate step lost rows or one of the new reader implementations
 ...
 * regression the Phase 6-E row-count invariants cannot: predicate drift
 ...
 * @throws {Phase6VerificationError} If any pair of totals disagrees.
 */
export async function checkReaderParity(trx: Knex): Promise<void> {
	const checks: {
		label: string;
		legacy: () => Promise<number>;
		phase6: () => Promise<number>;
	}[] = [
		{
		...
		},
		{
			...
			phase6: async () => scalar(trx('image_items').count({ count: '*' })),
		},
		{
			label: 'listResources total',
			legacy: async () => scalar(trx('resources').count({ count: '*' })),
			phase6: async () => scalar(trx('resource_items').count({ count: '*' })),
		},
		{
		...
		},
		{
			...
			phase6: async () => scalar(trx('analysis_violations').count({ count: '*' })),
		},
	];

	const failures: string[] = [];
	for (const { label, legacy, phase6 } of checks) {
		const [legacyValue, phase6Value] = await Promise.all([legacy(), phase6()]);
		if (legacyValue !== phase6Value) {
			failures.push(`${label}: legacy=${legacyValue}, phase6=${phase6Value}`);
		}
	}
