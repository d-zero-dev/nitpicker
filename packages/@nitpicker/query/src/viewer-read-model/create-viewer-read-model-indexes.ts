...
/**
 * Every index this read model needs, in creation order. Kept as a flat list
 * (rather than inline `await trx.raw(...)` calls) so
 * {@link createViewerReadModelIndexes} can report progress after each one
 * completes (issue #294) — without this, the whole multi-minute pass looked
 * like a single indistinguishable hang.
 */
const INDEX_STATEMENTS: readonly string[] = [
	...
	'CREATE INDEX vp_path ON viewer_pages(path_sort_key, page_id)',
	...
	// way.
	'CREATE INDEX vee_count ON viewer_error_kind_entries(count)',
	...
	'CREATE INDEX vi_default ON viewer_images(page_url_rank, image_id)',
	...
	'CREATE INDEX vi_width ON viewer_images(width, image_id)',
	'CREATE INDEX vi_height ON viewer_images(height, image_id)',
	...
	'CREATE INDEX vi_is_lazy ON viewer_images(is_lazy, image_id)',
 ...
 * @param onProgress - Called after each `CREATE INDEX` statement completes,
 *   with the count created so far and the total ({@link INDEX_STATEMENTS}'s
 *   length) — issue #294: on a large archive this whole pass can run for
 *   several minutes with no other signal that it hasn't hung. Omit for a
 *   silent build (the default; e.g. tests).
 * @example
 * await createViewerReadModelTables(trx);
 * // ...bulk-load every table...
 * await createViewerReadModelIndexes(trx);
 */
export async function createViewerReadModelIndexes(
	trx: Knex,
	onProgress?: (completed: number, total: number) => void,
): Promise<void> {
	const total = INDEX_STATEMENTS.length;
	let completed = 0;
