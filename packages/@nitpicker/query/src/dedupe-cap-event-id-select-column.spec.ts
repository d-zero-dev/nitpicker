...
const workingDir = path.resolve(
	__dirname,
	'__test_fixtures_dedupe_cap_event_id_select_column__',
);

describe('dedupeCapEventIdSelectColumn', () => {
	...
	it('selects the raw dedupe_cap_event_id column expression when the column is present', () => {
		const knex = archive.getKnex();
		expect(dedupeCapEventIdSelectColumn(knex, true)).toBe('dce.id as dedupeCapEventId');
	});
	...
	it('degrades to a NULL literal usable in a real select when the column is absent', async () => {
		...
		const row = await knex('content_items')
			.select(dedupeCapEventIdSelectColumn(knex, false))
			.first();
		expect(row.dedupeCapEventId).toBeNull();
	});
});
