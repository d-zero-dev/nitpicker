...
import { listReconcileRuns } from './list-reconcile-runs.js';

const __filename = new URL(import.meta.url).pathname;
const __dirname = path.dirname(__filename);
const workingDir = path.resolve(__dirname, '__test_fixtures_list_list_reconcile_runs__');

/**
 * Minimal archive config — listReconcileRuns reads from `list_reconcile_runs`
...
describe('listReconcileRuns', () => {
	let archive: InstanceType<typeof Archive>;

	beforeEach(async () => {
		archive = await buildArchive(`list-reconcile-runs-${Date.now()}.nitpicker`);
	});
	...
	it('returns rows ordered by `ran_at` DESC (newest first)', async () => {
		...
		const result = await listReconcileRuns(archive);
		expect(result.items.map((r) => r.list_label)).toEqual(['newest', 'mid', 'oldest']);
		expect(result.total).toBe(3);
	});

	it('honours `limit` and `offset` for pagination', async () => {
		await archive.recordListReconcileRun({ ran_at: '2026-06-21T00:00:00Z' });
		await archive.recordListReconcileRun({ ran_at: '2026-06-20T00:00:00Z' });
		await archive.recordListReconcileRun({ ran_at: '2026-06-19T00:00:00Z' });

		const firstPage = await listReconcileRuns(archive, { limit: 2 });
		expect(firstPage.items).toHaveLength(2);
		expect(firstPage.total).toBe(3);

		const secondPage = await listReconcileRuns(archive, { limit: 2, offset: 2 });
	...
	it('tolerates missing invalid_skipped / exclude_skipped columns (pre-#99 / pre-#260 archive, stub / stale tar-cache read)', async () => {
		...
		await archive.getKnex().schema.table('list_reconcile_runs', (t) => {
			t.dropColumn('invalid_skipped');
			t.dropColumn('exclude_skipped');
		});
		expect(
			await archive.getKnex().schema.hasColumn('list_reconcile_runs', 'invalid_skipped'),
		).toBe(false);
		expect(
			await archive.getKnex().schema.hasColumn('list_reconcile_runs', 'exclude_skipped'),
		).toBe(false);
	...
	it('returns an empty result when `list_reconcile_runs` is missing (read-only / legacy archive fallback)', async () => {
		...
		// "no such table: list_reconcile_runs" would break those flows.
		await archive.getKnex().schema.dropTableIfExists('list_reconcile_runs');
		const result = await listReconcileRuns(archive);
