 ...
 * legacy live union-find path). Takes {@link GetViewerIsolatedClusterOptions}
 * (fast-path shape, `status`/`source` array-capable): the legacy
 * `getIsolatedCluster` still filters both by strict equality, so a
 * multi-select value is narrowed to its first element via
 * `resolveLegacyFilterValue` on that branch — multi-select degrades to
