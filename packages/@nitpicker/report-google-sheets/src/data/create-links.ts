 ...
 * restricts both `estimateRowCount` and `run` to exactly those
 * `content_items` rows — matched on the joined `url_refs.url` column since
 * `content_items` itself has no `url` column of its own.
 * @param options - Row-set restriction. Omitted or `urls: undefined` reports every page.
 * @param options.urls - See above.
 * @returns A {@link CreateSheet} factory for the Links sheet.
 */
	...
	return (_reports, accessor) => {
		return {
			...
			async run({ sheet, maxRows, estimatedTotal, onProgress }) {
				...
				for await (const chunk of streamAllContentItems(accessor, { urls })) {
