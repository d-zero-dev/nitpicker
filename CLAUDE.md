...
> **0.13 format (write-model refactor, #103)**: `pages` / `anchors` / `images` / `resources` / `resources-referrers` / `pages.responseHeaders` は同名のまま残るが、reader は `content_items` / `page_meta` / `resource_items` / `anchor_edges` / `resource_ref_edges` / `image_items` の entity テーブルと `url_refs` / `text_refs` / `content_type_refs` / `json_refs` / `blob_refs` / `header_flags` の ref テーブルからしか読まない。書き込みパス（`Database.setPage` など）は次段の #196 で新テーブルへ切り替わるまで legacy テーブルを更新するので、reader 経由の見え方と書き込み経路が一時的にズレる（`packages/@nitpicker/query/src/__test-utils__/populate-migration-tables.ts` は spec だけがそのズレを吸収するために使う）。`assertCompatibleVersion` の要件は `info.version >= 0.13.0`。pre-0.13 archive（0.10.x / 0.11.x / 0.12.x）は `scripts/migrate-to-0.13.mjs` を通してから開く。pre-0.10 archive は `migrate-to-0.10.mjs` → `migrate-to-0.13.mjs` の順で 2 段実行する（`IncompatibleArchiveError` のメッセージが実行順を案内する）。**`REQUIRED_FORMAT_VERSION` は `pkg.version` と別値**: crawler が新規 archive の `info.version` に書き込むのは REQUIRED_FORMAT_VERSION（= フォーマット cut）であって、npm リリース番号ではない。リリース間で format cut しないパッチや minor では pkg.version が動いても `info.version` は据え置き、逆に format-breaking な PR は release 前でも REQUIRED_FORMAT_VERSION を先行して bump する。詳細は `packages/@nitpicker/crawler/src/archive/meta/assert-compatible-version.ts` の JSDoc を正とする。
