...
npx @nitpicker/cli crawl --append <file> <URL> [<URL>...]  # 既存アーカイブに新しい起点 URL を追加クロール
...
> **`--append <file>`**: 既存 `.nitpicker` を開き、位置引数の URL を新しい起点として追加クロールする。新スコープに該当する旧 external ページは internal として再スクレイプされる。失敗時は `<file>.bak` から自動復元、成功時は `.bak` 削除。`--resume` / `--diff` / `--output` / `--list` / `--list-file` / `--single` との同時指定は不可。
