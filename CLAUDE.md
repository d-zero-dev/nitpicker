...
> **Note (`/api/error-kinds` の viewer*error_kind*\* read model、issue #118)**: `getErrorKinds`（`page_errors`+`crawl_errors`/`error.log`を読み`classifyErrorKind`で毎回分類・集計）はGETのたびに再計算していたが、`buildViewerReadModel`内で1回計算した結果を4テーブル（`viewer_error_kind_groups`/`_hosts`/`_samples`/`_meta`）に正規化して書き込むよう変更した。`getErrorKinds`呼び出しは`viewer_summary`と同じ理由（再利用できる`sourceRows`が無い）で`knex.transaction()`開始**前**に`getSummary`と`Promise.all`実行する。**分類ロジックは重複させない**——`classifyErrorKind`は`getErrorKinds`内で1回だけ呼ばれ、`computeErrorKindInsertRows`は既に分類済みの`ErrorKindsResult`を4テーブル用の行配列に正規化するだけの純粋関数。`viewer_error_kind_samples.url`は`viewer_anchor_facts`と同じ理由（issue #139のref-table未実装）でinline text、design doc記載の`url_id`は採用しなかった。`total`/`channelSource`は`viewer_summary`と同じ単一行パターンで`viewer_error_kind_meta`に持たせる（3テーブルのどれにも属さない全体メタ値のため）。呼び出し元は共有ヘルパー`getErrorKindsFastPath`（`isViewerReadModelCurrent`ガード+`getViewerErrorKinds`/`getErrorKinds`二択）経由でdispatchし、viewer routeは`viewer_summary`と同型の`error-kinds-cache.ts`（LRU+ディスクキャッシュ+stub mode bypass）を追加で挟む。**tie-break順は未規定（groups・hostsとも）**——`getErrorKinds`はJSの`Map`挿入順、read modelは`ORDER BY count desc`＋`kind`/`host` asc（read model自身の出力を決定的にするための明示的tie-breakで、legacy挿入順に合わせる目的ではない）で、count同点時の並び順は両者で一致する保証が無く、`getErrorKindsFastPath`のdispatch先切替タイミングだけで表示順が変わりうる（テストは`kind`でソートしてから比較）。スキーマ変更のため`VIEWER_READ_MODEL_SCHEMA_VERSION`を7→8にbump。40万件規模の合成データ実測で直接呼び出し比較 `getErrorKinds` 約818ms → `getViewerErrorKinds` 約1.4ms（約580倍）、`/api/error-kinds` HTTP cold 約1011ms → 約2.3ms。詳細は ARCHITECTURE.md の `@nitpicker/viewer` 節「設計注意（`/api/error-kinds` の viewer*error_kind*\* read model、issue #118）」を正とする。
