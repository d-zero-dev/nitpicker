 ...
 * `descendant_html_page_count` existed (read model schema version < 17,
 * bumped when the directory-tree Sankey diagram's HTML-only page counts
 * were added). No archive write-model change accompanies this bump — only
 * `VIEWER_READ_MODEL_SCHEMA_VERSION` moved — so this is a plain rebuild via
 ...
 * the same way `migrate-to-0.10.mjs` / `migrate-to-0.13.mjs` record theirs.
 *
 * USAGE
 * -----
 *
 *     node scripts/migrate-to-0.14.mjs <old.nitpicker> [<new.nitpicker>]
 *
 * If <new.nitpicker> is omitted, writes to `<old>.0.14.nitpicker` next to
 * the input. Mirrors `migrate-to-0.13.mjs`'s own contract: the input file
...
/**
 * Reports a progress line to stdout, indented like `migrate-to-0.13.mjs`'s
...
async function main() {
	...
	if (!inputArg) {
		console.error(
			'Usage: node scripts/migrate-to-0.14.mjs <old.nitpicker> [<new.nitpicker>]',
		);
		process.exit(1);
	}
	...
	const outputPath = path.resolve(
		outputArg ??
			path.join(
				...
				`${path.basename(inputPath, path.extname(inputPath))}.0.14.nitpicker`,
			),
	);
