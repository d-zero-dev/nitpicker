...
>
> **`getViewerErrorKinds`はlegacyと同じoptions契約をSQL側で実装**: `host`/`kind`フィルタは`WHERE`、`sortBy`/`sortOrder`は`ORDER BY`（`host_sort_key`/`kind_sort_key`で決定的tie-break付き）、`limit`/`offset`はSQLの`LIMIT`/`OFFSET`（`limit`省略時は`LIMIT -1`＝無制限、legacyの「limit省略で全件」契約に合わせる）で処理する。`total`は`COUNT(*)`をフィルタ適用後・`LIMIT`適用前に取る一方、`facets`（`totalRecords`/`channelSource`）はフィルタ非依存の`viewer_error_kind_meta`から取るため、フィルタしても総件数の表示が動かない。
>
> **tie-break順は未規定**: `getErrorKinds`のitems配列は`count`降順にソートするが、同countの場合の並び順はJSの`Map`挿入順（`page_errors`→`crawl_errors`の処理順に依存する偶発的な順序）に委ねられている。read model側の`getViewerErrorKinds`は`ORDER BY count desc`＋`host_sort_key`/`kind_sort_key` asc（read model自身の出力を決定的にするための明示的tie-breakであり、legacy側の挿入順と一致させる目的ではない）で読むため、同count時の並び順はlegacy経路の挿入順と一致する保証が無い。両者とも「countで降順」という契約のみを守ればよく、同count時の順序一致は`ErrorKindsResult.items`の型定義上も約束されていない——`getErrorKindsFastPath`は read model の staleness に応じて2経路を透過的に切り替えるため、データが変わらなくても切替タイミングだけで同点entryの表示順が変わりうる（表示順に依存する運用が必要な場合は呼び出し側で追加のソートキーを適用すること）。テスト（`get-viewer-error-kinds.spec.ts`/`build-viewer-read-model.spec.ts`）は比較前に`host`+`kind`でソートして正規化している。
>
> **dispatchは`getSummaryFastPath`と同型、キャッシュは「options非依存の全件スナップショット」だけを持つ**: `getErrorKindsFastPath`（`isViewerReadModelCurrent`ガード + `getViewerErrorKinds`/`getErrorKinds`の二択、`options`をそのまま透過）に集約し、viewer route（`register-error-kinds-route.ts`）はさらに`error-kinds-cache.ts`を経由する。`summary-cache.ts`と異なり`/api/error-kinds`は`host`/`kind`/`sortBy`/`sortOrder`/`limit`/`offset`というリクエストパラメータを持つため、素朴に「結果をarchiveId単位でキャッシュ」すると別のクエリパラメータの組み合わせに対して誤ったキャッシュ結果を返してしまう。そこでキャッシュ層は`getErrorKindsFastPath(accessor, {})`（オプション無し＝全件）の結果だけをarchiveId単位でLRU/ディスクキャッシュし、実際のリクエストの`host`/`kind`/`sortBy`/`sortOrder`/`limit`/`offset`は`applyErrorKindsOptions`（純粋関数、`getErrorKinds`と同じフィルタ/ソート/ページネーション契約をJSで再現）でリクエストごとに適用する。これにより異なるクエリパラメータの組み合わせでも、legacy fallback時の高コストな分類走査（後述ベンチマークで約658ms）は1回のみで済む。**stub modeは`getErrorKindsFastPath`を使わず常に`getErrorKinds`を実際の`options`付きで直接呼ぶ**（キャッシュも`applyErrorKindsOptions`も経由しない）——`summary-cache.ts`と同じ理由（`crawl --resume`/`--append`/`--retry-failed`が再オープンするtmpDirが既にread modelを持つ場合、`isViewerReadModelCurrent`のスキーマバージョン一致判定だけでは再開前の古いスナップショットを誤って返しかねない）。スキーマ変更のため`VIEWER_READ_MODEL_SCHEMA_VERSION`を8→9にbump（旧`groups/hosts/samples/meta`4テーブル時代の7→8を経て、dev側の破壊的変更への追従でさらにreshape）。
>
> **ベンチマーク実測**（`scripts/bench-viewer-error-kinds-read-model.mjs`、synthetic archive、実顧客データ不使用）: `crawl_errors`40万行規模（DNS/timeout/connection-refused/tlsの4kindに偏りを付けた合成分布、50ホスト、host×kindペアは高々200件程度に収束）で、read model build time 約552ms。直接関数呼び出し比較で`getErrorKinds`（legacy）約658ms → `getViewerErrorKinds`（read model）約2.1ms（約310倍）。`EXPLAIN QUERY PLAN`はフィルタ無し/`kind`フィルタとも`SCAN viewer_error_kind_entries USING INDEX vee_count`、`host`+`kind`同時指定（detail pane用）は`SEARCH ... USING PRIMARY KEY`。HTTP経由の`/api/error-kinds`はcold約691ms→約4.4ms、warm p50はキャッシュ層により両経路ともsub-ms（read modelの純粋な寄与は上記の直接呼び出し比較値で見るのが正しい、`summary-cache.ts`と同じ注意点）。
