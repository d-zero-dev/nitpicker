...
- **enum フィルターの複数選択（チェックリスト UI）は fast path のみ OR 結合、legacy path は先頭要素へ縮退する** — `c.req.queries()` で受けた配列は fast path（`apply-equality-or-in-filter.ts`。`whereIn`、500 件超は chunk 分割）でのみ OR 結合される。read model 非経由の legacy クエリ関数は単一値の等価比較しか持たないため、`resolve-legacy-filter-value.ts` が配列の先頭要素だけを渡す（read model 未構築・stub mode・強制 legacy 条件のいずれかで legacy に落ちた場合、複数選択は単一選択に縮退する — エラーにはしない）。空配列は「フィルタ解除・全件表示」（`has-filter-value.ts`、`matches-any-filter-value.ts`）。mismatches-view のみ既定値 `'canonical'` へのフォールバックという固有ルールを持つ（`get-mismatches-fast-path.ts` の `resolveLegacyType`）
