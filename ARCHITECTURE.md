...
- **fast path の強制 live 条件は関数ごとに異なる** — pages/resources は `urlPattern`/`directory`、broken links は `urlPattern`/`includeRedirectSources`、headers/mismatches は明示 `sortBy`、duplicates のみ強制 live なし。各 `get-*-fast-path.ts` の JSDoc が正。duplicates の live 経路は実 `group_id` と衝突しない負の sentinel `-(index+1)` を採番し、route は `groupId <= 0` を 404 にする
- **enum フィルターの複数選択（チェックリスト UI）は fast path のみ OR 結合、live path は先頭要素へ縮退する** — `c.req.queries()` で受けた配列は fast path（`apply-equality-or-in-filter.ts`。`whereIn`、500 件超は chunk 分割）でのみ OR 結合される。read model 非経由の live クエリ関数は単一値の等価比較しか持たないため、`resolve-live-filter-value.ts` が配列の先頭要素だけを渡す（read model 未構築・stub mode・強制 live 条件のいずれかで live path に落ちた場合、複数選択は単一選択に縮退する — エラーにはしない）。空配列は「フィルタ解除・全件表示」（`has-filter-value.ts`、`matches-any-filter-value.ts`）。mismatches-view のみ既定値 `'canonical'` へのフォールバックという固有ルールを持つ（`get-mismatches-fast-path.ts` の `resolveLiveMismatchType`）
...
自己生成される無限 URL 空間（pagination 予測の暴走）への対処と、opt-in `--dedupe-cap` による同一メタデータクラスタの検出。前者は常時有効、後者は明示フラグでのみ動く。
...
7. CLI フラグ: `cli/src/commands/crawl.ts`（`--dedupe-cap` / `--dedupe-map-cap`）→ `cli/src/crawl/map-flags-to-crawl-config.ts`
...
10. viewer: `template-clusters` を軽量パターンの参照実装として使用（`register-duplicate-clusters-route.ts` + `duplicate-clusters-cache.ts`、`register-dedupe-cap-events-route.ts` はキャッシュなし・`network-outages` 型）。専用ナビ項目は `dedupe_cap_events` には作らず Duplicate Clusters ビュー内の通知として表示
...
5. スキーマを変えたら `VIEWER_READ_MODEL_SCHEMA_VERSION` を bump（旧 read model は `isViewerReadModelCurrent` で自動的に live path へ落ちる）
