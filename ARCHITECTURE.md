...
3. `core/src/template-classification/classify-page-templates.ts`（`resolvePageClusterKeys` 呼び出し・`clusterKey`→`templateKey`読み替え・アーカイブ path+size+mtime+ページ数キーの永続キャッシュ）。`onClusterReason` コールバックでクラスタ確定ごとの `ClusterReason`（`@d-zero/page-cluster@0.5.0+`）も収集し、`{ templateKeysByUrl, clusterReasons }` を返す。`ClusterReason` 型自体は `@d-zero/page-cluster` の `package.json` `exports` に個別公開されていないため、`ResolvePageClusterKeysOptions['onClusterReason']` から `Parameters<>` で構造的に導出（非公開内部を直接 import しない）。永続キャッシュも両方をまとめて保存し、クラッシュ再開時に `clusterReasons` だけ空になる不整合を防ぐ
4. `crawler/src/archive/create-adjunct-tables.ts`（`page_templates` テーブルの DDL。`page_id` PK 1:1、`WITHOUT ROWID`。加えて `page_template_cluster_reasons`＝`ClusterReason` をクラスタ単位で保持する隣接テーブル、`template_key` PK・FKなし・`WITHOUT ROWID`。新規テーブルなので `initSchema` が毎回無条件実行する `createAdjunctTables` の `hasTable` ガードだけで既存アーカイブにも自動追加され、専用 migration script は不要）と `crawler/src/archive/db-ops/analysis/replace-page-templates.ts`（`page_templates` と `page_template_cluster_reasons` を同一トランザクションで全置換書き込み。URL が `content_items` に解決できないページは `replaceAnalysisViolations` と異なりエラーにせずスキップ。`ClusterReason` は `crawler` が `core`/`page-cluster` に依存しないため `archive/types.ts` の構造的ミラー型 `ClusterReasonData` で受け取る）+ `archive.ts` / `database.ts` のファサードメソッド
5. `core/src/nitpicker.ts`（`analyze()` の Phase 3。`getPagesWithRefs` のバッチコールバック内ではなく、全バッチ蓄積後に1回だけ実行する — バッチ境界を跨いだテンプレートキーの一貫性を保つため。分類結果は `archive.replacePageTemplates(templateKeysByUrl, clusterReasons)` で直接 SQL に書き込み、`Table`/`Report` には一切乗らない）
...
9. クラスタ分析: `query/src/list-page-template-clusters.ts`（`templateKey` ごとのページ数・共通ディレクトリ（`compute-directory-distribution.ts`）をクラスタの実メンバーページから再計算し、`query/src/read-cluster-reasons-by-template-key.ts` で `page_template_cluster_reasons` から `ClusterReason` を `template_key` キーで読み戻す。`templateKey` 文字列自体は `@d-zero/page-cluster` のブロッキングキーで、`css:` 以下は SHA-256 ハッシュのため人間には読めない — これがクラスタ単位の再計算が必要な理由。`TemplateClusterSummary.reason` は `page_template_cluster_reasons` に対応行が無ければ `null`（`page-cluster@0.3.1` 時代に分類された既存クラスタの意図的なフォールバック — `hasClassification: false`＝`--templates` 未実行とは別概念）。共通スタイルシートのファイル名は `reason.blocking` の `kind: 'css'` エントリの `distinctiveStylesheetHrefs`（`@d-zero/page-cluster` が実際にハッシュ化前に適用する first-party フィルタ・文書頻度90%フィルタを通した正確な値）から `compute-stylesheet-file-names.ts` で導出するのみで、CSS積集合を独自に再計算する処理は持たない — `ClusterReason` が正確な値を運んでくるため、非公開実装の近似再現は不要。`page_templates` はどのアーカイブにも `createAdjunctTables` で常時作成されるため、`hasClassification`（`--templates` 実行済みか）はテーブル有無ではなく行数0件で判定する）
10. viewer 表示（クラスタ分析画面）: `viewer/src/routes/register-template-clusters-route.ts`（`GET /api/template-clusters` 一発取得。`page_templates` は read model 非経由のためキャッシュは `viewer/src/template-clusters-cache.ts` のメモリ LRU のみ、stub mode はbypass）+ `viewer/web/routes/template-clusters-view.tsx`（`templateKey` ごとに `<details>` セクション表示、見出しは共通CSSファイル名優先・無ければ共通ディレクトリにフォールバック、`reason` があれば blocking（css/path/orphanMerge の内訳）・structuralCoreTokens・landmarks（型ごとの presenceRate/chromeRate/shellTokens）・siblingClusterKeys（同ページ内の対応クラスタへのアンカーリンク）を追加セクション表示、`reason: null` ならそのセクションのみ再実行案内に切り替え、Pages 一覧への `templateKey` フィルタ付きリンク）。MCP 専用ツール・CLI サブコマンドは意図的に未実装
