...
- **`listLinks`**: リンク分析（`type: 'broken' | 'external'`、anchor 単位 = 1 行 1 `<a>` タグ、重複排除なし）。dest は `pages.redirectDestId` 経由で canonical destination まで解決した上で broken/external 判定（`includeRedirectSources: true` で解決を無効化し literal を見る）。関数自体は変更していないため CLI/MCP は従来通り `type: 'broken' | 'external'` で anchor 単位の生データを取得できる。viewer 側は `type: 'external'` は `listExternalLinks`/`listViewerExternalLinks`、`type: 'broken'` は `listViewerBrokenLinks` が current な read model を持つ場合に切り替わり（後述）、この関数は両方の legacy フォールバックとしてのみ残る
...
> **設計注意（viewer プロセス側 precompute cache）:** 10 GB scale archive で **isolated-\* 系 (20-30 s)** の wall-clock を SQL 単独で詰めるのは denorm 列（`canonicalId` / `isolated_root`）導入が必須、しかし schema 変更は consistency / migration / archive 互換性リスクが大きく本 PR では避ける判断。代替として viewer プロセス側で per-archive の memoised computation を持つ:
>
...
>
> LRU helper `packages/@nitpicker/viewer/src/promise-lru.ts` (`createPromiseLru`) を使う。Promise 単位で cache (concurrent 初回 request 1 回 computation で済む)、**read promote-on-read で真の LRU** (`Map.set` の existing key は insertion order を更新しないので `delete` + 再 `set`)、rejected promise は cache から落として retry 可能、max 4 entry。
>
> **stub mode (live crawl) bypass**: `context.mode === 'stub'` の archive は writer が in-place で anchors / pages を書き続けるため、cache snapshot を返すと**永久 stale** になる (`/api/links` と数字が食い違う等)。`getCachedIsolatedClusters` は stub mode で **cache を経由せず毎回 union-find** を再計算する (slow but live)。
>
> Query API 側は `listIsolatedPages` / `listIsolatedClusters` / `getIsolatedCluster` に `precomputedComponents?` option を追加。viewer route が cache から取って渡す。CLI / MCP は option を渡さないので従来の SQL 経路を踏む (一回呼び切りなので precompute コストを payback できないため、これは意図通り)。`get-isolated-cluster` の 404 は singleton（`/api/isolated-pages` を使うべき URL）と collapsed cluster で別メッセージに分けて、deep-link / 共有 URL の混在で「実は singleton でした」を判別可能にしている。
>
> **scope 外**: `/api/links?type=broken` 19 s / `/api/summary` 3.6 s / `/api/duplicates` 3.5 s / `/api/images` 2.6 s は本 PR で改善せず accept。`listLinks` は anchor-scan bound (`SCAN anchors → rowid seek source / dest / canonical`)で、denorm 列 `canonicalId` を入れても JOIN 経路の dominant cost である anchor scan が残るため意味が薄い。残り 3 endpoint は covering index 追加で詰める余地ありだが、PR #96 教訓（status + anchor の 7 個 bulk 追加が listLinks/Graph を 30-50x 回帰させた）に従って index 追加は最大 2 個までと自制したい — 本 PR では加えず別 issue 候補。**検索キーワード**: 「viewer endpoint 遅い」「isolated 28s」「precomputed cache viewer」「stub mode stale」。
>
> **旧 `referrer-count-cache.ts` / `/api/page-links` / `listPageLinks`（本節が元々説明していたもう一方の cache）は「ページリンク」ビューの廃止に伴い削除済み。**per-page の被リンク数・リダイレクト元数は Page Detail ビュー（`/api/pages/detail`）の inbound/outbound/redirectFrom で個別ページ単位に確認する。
...
> **設計注意（ページネーションは MPA をデフォルト・仮想スクロールを opt-in にする）:** リスト系ビュー（pages / resources / images / broken-links / external-links / violations / isolated-pages / isolated-clusters）はすべて `DataTable` 経由で `usePaginationMode()` を読み、MPA（`PagedTable` + `?page=` / `?pageSize=` URL クエリ）と virtual scroll（`VirtualTable` + `useInfiniteQuery`）を切り替える。**MPA をデフォルトにしている理由は (a) deep-link / URL 共有 / ブラウザ戻る・進むが効くこと、(b) クライアント / ディレクター用途では特定ページに直行できる UX が好まれること**。virtual scroll は 10 万行規模の探索性が必要なときの opt-in。両モードとも backend は同じ `limit`/`offset` API で、view 側で `usePagedQuery` / `use-*-infinite` のどちらかを `enabled` フラグで起動する。

> **設計注意（`/api/violations` の SQL read path、issue #116）:** 新規 analyze は `analysis/violations.json` と `analysis/report.violations` を保存せず、`analysis_violations` に違反行、`analysis_text_refs` に `message` / `code` を保存する。`analysis_text_refs` は #116 専用で、#139 の shared `text_refs` / `url_refs` を待たずに request-time JSON scan を止めるための局所的な重複排除テーブル。旧 archive 互換は request-time fallback ではなく、`nitpicker viewer-build` と crawl 完了時の read-model build が SQL table 空の場合だけ `analysis/violations.json` を backfill する。read-only open / stub mode では schema mutation や backfill をしない。合成ベンチ `node scripts/bench-analysis-violations.mjs 50 1000` では `getViolations default` 1.56ms（`av_url_order` covering scan）、combined filter 0.49ms（`av_filter_url` covering search）。
...
>
> **read model のビルドタイミング（issue #112、issue #177 で3経路→2経路に是正）は2経路**:
>
...
>    **削除された旧3経路目（on-open opportunistic build）**: #112 は当初、`Archive.openCached` 経由の read-only open 時に `isViewerReadModelCurrent` が false なら `archive-manager.ts` の `#performOpen` がその場でタールキャッシュ dir に書き込んでビルドする第3の経路（`ensureViewerReadModelOpportunistically`）を持っていた。実アーカイブ（17万ページ規模）での検証で、単純な read クエリが初回 20 分以上ブロックされる・disk 逼迫時にビルド失敗がタールキャッシュ自体を破損させる（`database disk image is malformed`）・失敗を検知するサーキットブレーカーが無く同じアーカイブを開くたびに同じビルドを再試行し続ける、という実害が判明したため issue #177 で全面削除した。read-only open は書き込み・ビルドを一切行わない設計に統一し、read model が無い/古いアーカイブは明示的な `viewer-build` を運用者が実行するまで恒久的に legacy 経路のまま — というのが受容された定常状態。
>
...
> **設計注意（Pages facet の事前計算、issue #106）:** `/api/pages` の Status / Language / Scope ドロップダウンが表示する動的 enum 候補（`PageListFacets`）は、legacy path (`listPages`) では毎リクエスト `pages` 全体への `DISTINCT status, lang, isExternal` スキャン（`list-pages.ts` の `getPageListFacets`）で作っていたが、fast path (`listViewerPages`) にはこの集計が存在せず facet が常に空になっていた（`dev` の PR #148 が facet 依存の動的フィルタ UI を追加した際に、`feature/impl-new-db` の fast path 側が未対応のまま残っていた統合バグ。`viewer.spec.ts` の facet 系 e2e 2 件が実際に red だったことで発覚）。対応として、`buildViewerReadModel`（`viewer-read-model/build-viewer-read-model.ts`）が `pages` 走査と同じ1パスで `computePageFacetBuckets`（`viewer-read-model/compute-page-facet-buckets.ts`）を呼び、`content_category` ごと（実カテゴリ18種 + `html`∪`unknown` の合成カテゴリ `default`）に `status` / `lang` / `is_external` の distinct 値を `viewer_count_buckets` へ `key='facet:<dimension>:content_category=<category>'` 形式で書き込む。読み出し側 `readViewerPageFacets`（`read-viewer-page-facets.ts`）は `(scope, key, value)` の PK point lookup のみで、GET 時に `pages` を一切スキャンしない。**スキーマ変更は不要**（既存 `viewer_count_buckets` の再利用）だが、population logic が変わるため `VIEWER_READ_MODEL_SCHEMA_VERSION` を 2→3 に bump し、旧バージョンの read model は自動再ビルド対象にした。400k 行 synthetic archive 実測（後述）で facet 集計自体は build time 11.9s 中 177ms（1.5%）、追加行数はカテゴリ×次元の組み合わせ分のみ（今回の均質な合成データで 25 行、実アーカイブでも高々 category 数 × 数十〜数百程度）で DB サイズへの影響は無視できる。
...
>
> 1. **`viewer_url_refs`（issue #139）で URL 文字列を共有する**: #114 実装時は `source_url_sort_key`/`dest_url_sort_key` を inline text としていたが、#139 で `viewer_url_refs(id, url)` を追加し、`viewer_anchor_facts.source_url_ref_id`/`dest_url_ref_id` と `viewer_external_links.dest_url_ref_id` が参照する形へ移行した。`viewer_url_refs.id` は `pages.url` を SQLite BINARY 相当の URL 昇順で採番するため、compact な整数 sort key としても使う。full URL 文字列は broken/external の row window を limit した後に `viewer_url_refs` から復元し、API の `sourceUrl`/`destUrl` 形状は変えない。
> 2. **`viewer_external_links` はテーブルとして分離したまま維持し、`viewer_anchor_facts` から1回のスキャンで導出する**: `viewer_anchor_facts` は `(source_page_id, dest_page_id)` ペア単位でdedup済みのedgeテーブルなので、宛先ごとのreferrerCountは「そのdest_page_idを持つedge行の数」を数えるだけで求まる（edge単位で既にdedup済みなので `COUNT(DISTINCT source)` と数学的に同値）——`GROUP BY`のランタイム再導入は不要。`compute-anchor-fact-rows.ts` が `anchors` を`source.id`の範囲ごとにchunk化してスキャンし（大規模アーカイブで`anchors`を一括ロードするとOOMする問題への対処、詳細は同ファイルのJSDoc）、chunkごとの結果（メモリ上の配列）から `derive-external-link-summary-rows.ts`（純粋関数、DBアクセス無し、chunkごとに独立して集計しchunk跨ぎの状態は持たない）が `viewer_external_links` 行を導出する。chunk跨ぎの`referrer_count`合算はJS側のアキュムレータではなく、`upsert-external-link-rows.ts`が`INSERT ... ON CONFLICT (dest_page_id) DO UPDATE SET referrer_count = referrer_count + excluded.referrer_count`でSQLite側に委譲する（JSでchunk跨ぎに集計値を持ち回ると、distinct外部宛先が多いアーカイブでその集計自体が無制限にメモリを消費し、chunk化した意味が失われるため）。`anchors`をスキャンするのはこの関数のみ、という不変条件はchunk化後も変わらない。旧 `compute-external-link-rows.ts`（独自の2回目の `anchors` スキャン）は廃止。テーブルを統合して edge 単位一本化すると External Links ビューの「宛先ごとの参照元数」というUXが失われる（PR #153 のUX決定を破壊する）ため、2つの独立したテーブルとして残す判断をした。
...
>
> **スキーマ**: `viewer_url_refs(id PK, url UNIQUE)`、`viewer_anchor_facts(edge_id PK, source_page_id, dest_page_id, source_url_ref_id, dest_url_ref_id, status, status_sort_key, status_desc_key, count, is_broken, is_external_link)`、`viewer_external_links(dest_page_id PK, dest_url_ref_id, status, referrer_count)`。`is_external_link` は永続化するが（SQLite の INTEGER 0/1 は実質無コスト）index は張らない——read時にこの列でフィルタするクエリは存在せず、build時の `deriveExternalLinkSummaryRows` の in-memory pass でのみ使われるため。`status_desc_key`（`viewer_pages` と同じ負数キー）が必要な理由: Stable Ordering 規則は `status desc` でも `source_url_ref_id`/`edge_id` のタイブレークを ASC のまま保つが、row-value keyset タプル比較は列ごとに方向を混在させられないため、主キーを負数化して常に ASC スキャンにする。
>
> **#139 ベンチマーク**: `node scripts/bench-viewer-url-refs.mjs` は同一 synthetic edge set で旧 inline URL 形と `viewer_url_refs` 形を別 DB に作り、サイズ・URL 復元込みの代表 query・`EXPLAIN QUERY PLAN` を比較する。既定の 50,000 synthetic edges は URL を 10,000 distinct に丸めた「重複が多い」条件なので、容量削減率は一般化せずこの条件の上限寄りの目安として扱う。この条件では inline 14.1 MiB → ref 4.7 MiB（66.3% 削減）。broken sourceUrl sort は inline 0.16ms / ref 0.20ms p50、external destUrl sort は inline 0.13ms / ref 0.14ms p50。broken は window 後に URL ref を主キー lookup、external は `vel_url` scan 後に URL ref を主キー lookup する plan になり、ref 化しても代表 read path のオーダーは保てている。実 archive path / site name は使わない。
>
> **cursor pagination**: `viewer-anchor-facts-cursor/` は `viewer_pages` と同じ shared cursor kit を使い、`source_url_ref_id`/`dest_url_ref_id`/`status_sort_key`/`status_desc_key`/`edge_id` の数値タプルで keyset pagination する。`listViewerBrokenLinks` は `listViewerPages` と同じ4系統（初回/forward keyset/backward keyset/offset直読み）を実装し、最後に limit 済みの ref id だけ `viewer_url_refs` へ lookup して従来の `LinkEntry` に戻す。
>
...
> **設計注意（`/api/summary` の viewer_summary read model、issue #104）:** `getSummary`（`pages`への3並列SQL集計 + `status=-1`行の`resolveFailedPageMessages`によるerror kind内訳計算、428k行規模で~22s）はGETのたびに再計算していた。`buildViewerReadModel`内で`getSummary(accessor)`を1回計算し、単一行テーブル`viewer_summary`（`total_pages`/`internal_pages`/`external_pages`/`internal_contents`/`external_contents`のスカラー列 + `status_json`/`content_type_json`/`metadata_json`のJSON文字列列、`viewer_read_model_meta`と同じ`id integer primary key check (id = 1)`パターン）にキャッシュする。`baseUrl`/`roots`は`accessor.getConfig()`由来でpages集計と無関係なため列に含めない。`docs/viewer-sql-query-plan.md`が前提とする`url_refs`/`content_items`ベースのref-table設計（issue #139、未着手）は採らず、`viewer_anchor_facts`と同じ理由で`pages`直接参照・素朴なJSON列の規約に合わせた。`/api/error-kinds`用の`viewer_error_kind_groups/hosts/samples`（同docsのPhase 4 2番目の項目）は別issue・別エンドポイントでありスコープ外——`status=-1`のerrorKindBreakdownは`status_json`に埋め込んで完結させる（issue #118で後述の`viewer_error_kind_*`テーブルとして実装済み）。
>
> **`getSummary`呼び出しをtransaction外に置く理由**: `viewer_anchor_facts`/`viewer_pages`は`sourceRows`/`anchors`の二重スキャンを避けるためtrx内で読むが、`getSummary`には再利用できる共有rowsが無くtrx内に置く性能上のメリットが無い。加えてtrx開始前に計算することで、`getSummary`失敗時に`dropViewerReadModelTables`が一切走らず旧read modelを壊さずに済む（trx内で失敗させるとtrxロールバック境界が曖昧になる）。この事前読み取りが安全なのは、`getSummary`が読む`pages`/`page_errors`/`crawl_errors`/`error.log`のいずれも、read model build自体が書き込まない、かつ2つの構築経路（crawl完了時・`viewer-build`）はいずれもアクティブなscraperと同時実行されないため。
>
> **`accessor.getConfig()`への新規依存**: `getSummary`は`accessor.getConfig()`解決を要求するため、`buildViewerReadModel`にとって新しいprecondition になった。`CrawlerOrchestrator`は`crawling()`開始前に必ず`archive.setConfig(...)`を呼ぶため、実クロールを経た全archiveでconfigは保証される。`Archive.create`の`initSchema`はデフォルトconfig行を自動seedしないため、`setConfig`実行前に壊れた/手動作成された`.nitpicker`を渡した場合は`buildViewerReadModel`が`Error('No config')`で失敗するが、2つの構築経路のいずれもそのようなarchiveを対象にしない前提の受容済みtrade-off。
>
> **dispatchは共有ヘルパーに集約**: `register-pages-route.ts`/`register-links-route.ts`はroute層1箇所のみでfast path/legacyを分岐するが、summaryは呼び出し元が3〜4箇所（viewer route経由の`summary-cache.ts`のstub/archive両分岐、CLI `query summary`、MCP `get_summary`/`open_archive`）で完全に同一の「read modelが currentか否か」の二択ロジックを繰り返すため、`getSummaryFastPath`（`isViewerReadModelCurrent`ガード + `getViewerSummary`/`getSummary`の二択）に集約した。**stub modeは`getSummaryFastPath`を絶対に使わない**——stubのtmpDirは`crawl --resume`/`--append`/`--retry-failed`で再オープンされた、既に一度crawl完了しread modelを持つ旧archiveであり得るため、`isViewerReadModelCurrent`がスキーマバージョンの一致だけで「current」と誤判定し、writerが追記中のライブ集計ではなく再開前の古いスナップショットを返しかねない。stub modeは常に`getSummary`を直接呼ぶ。スキーマ変更のため`VIEWER_READ_MODEL_SCHEMA_VERSION`を6→7にbump。
>
> **ベンチマーク実測**（`scripts/bench-viewer-summary-read-model.mjs`、synthetic archive、実顧客データ不使用）: 40万ページ規模で、read model build time 約18.5秒、追加DBサイズ約294 MiB。直接関数呼び出し比較で `getSummary`（legacy）約345ms → `getViewerSummary`（read model）約1.4ms。`EXPLAIN QUERY PLAN`は`SEARCH viewer_summary USING INTEGER PRIMARY KEY`（単一行lookup）。HTTP経由の`/api/summary`はviewer-process LRU（`summary-cache.ts`）が初回ヒット後は常にsub-ms応答になるため、read modelの純粋な寄与は上記の直接呼び出し比較値で見るのが正しい。後述の「受容済みの遅い query」に挙げていた`getSummary`22sはこのPRで解消。

> **設計注意（`/api/error-kinds` の viewer*error_kind*\* read model、issue #118）:** `getErrorKinds`はGETのたびに`page_errors`+`crawl_errors`/`error.log`を統合し`classifyErrorKind`で分類・集計していた。#118と並行してdev側に別の破壊的変更（host×kindペア単位への正規化）が入り、`ErrorKindsResult`は旧`{total, channelSource, groups: {kind, count, hosts, sampleUrls}[]}`から現行`{items: ErrorKindEntry[], total, facets: {totalRecords, channelSource}}`（`ErrorKindEntry = {host, kind, count, sampleUrls, overflowedCount}`）に変わり、`getErrorKinds`自体が`host`/`kind`フィルタ・`sortBy`/`sortOrder`・`limit`/`offset`を引数に取るようになった。read model はこの正規化後の形に合わせて実装し直した：`create-viewer-read-model-tables.ts`には issue #104 実装時点で「error-kind用テーブルは別issueで実装する」という先行コメントが残されており、本issueでそれを解消する形で `viewer_error_kind_entries`（`(host, kind)` 複合PK・`WITHOUT ROWID`、`sample_urls_json`は`JSON.stringify`した配列——samplesは常に所属rowと一緒に読まれ独立にフィルタ/ソートされないため別テーブルへの正規化に意味がない）/ `viewer_error_kind_meta`（`viewer_summary`と同じ`id integer primary key check (id = 1)`単一行パターンで`total_records`/`channel_source`を保持）の2テーブルを追加した。`docs/viewer-db-redesign-plan.md`が前提とする`url_id`（`url_refs`FK、issue #139未着手）は`viewer_anchor_facts`/`viewer_summary`と同じ理由で採らず、`sample_urls_json`へのインライン格納に変更している。**indexは`count`用の`vee_count`一本のみ**——このテーブルの行数はdistinct(host)×distinct(kind)に収まり実運用でも数千行程度が上限のため、`host`/`kind`のテキストソートはフルスキャン+ソートでも十分高速という判断で、`viewer_pages`のような多数の専用indexは意図的に作らなかった（過剰indexを避ける根拠は issue #106 の「実測してからindexを足す」原則を踏襲——将来ベンチマークで問題が見つかれば追加する）。
>
> **分類ロジックを1箇所に保つ設計**: `buildViewerReadModel`内で`getErrorKinds(accessor)`を常にオプション無し（＝フィルタ無し・全件・count降順）で1回呼び（`getSummary`と`Promise.all`でtrx開始前に並列実行——`viewer_anchor_facts`と異なり再利用できる`sourceRows`が無く、trx内に置く性能上のメリットが無いのは`getSummary`と同じ理由）、その結果（既に`classifyErrorKind`で分類済みの`ErrorKindsResult.items`）を`computeErrorKindInsertRows`（純粋関数、DBアクセス無し）が`viewer_error_kind_entries`用の行配列に正規化するだけに留めている。`classifyErrorKind`自体は`getErrorKinds`内で1回しか呼ばれず、read-model buildが独自に再分類することは無い——「分類ロジックを複数箇所に重複させない」規約を踏襲。
>
> **`getViewerErrorKinds`はlegacyと同じoptions契約をSQL側で実装**: `host`/`kind`フィルタは`WHERE`、`sortBy`/`sortOrder`は`ORDER BY`（`host`/`kind` ascでの決定的tie-break付き——host/kindはこのコードベースのどこでもcase-foldingされないため、`host_sort_key`/`kind_sort_key`のような別列は持たずbase列を直接`ORDER BY`する）、`limit`/`offset`はSQLの`LIMIT`/`OFFSET`（`limit`省略時は`LIMIT -1`＝無制限、legacyの「limit省略で全件」契約に合わせる）で処理する。`total`は`COUNT(*)`をフィルタ適用後・`LIMIT`適用前に取る一方、`facets`（`totalRecords`/`channelSource`）はフィルタ非依存の`viewer_error_kind_meta`から取るため、フィルタしても総件数の表示が動かない。**sortBy検証とtie-breakは`resolveErrorKindsSort`/`sortArrayItems`（`@nitpicker/query`から公開）に一本化**——`getErrorKinds`・`getViewerErrorKinds`・viewer routeの`error-kinds-cache.ts`の3箇所が独立に「未検証の`sortBy`から`sortOrder`のデフォルトを算出する」ロジックとソート処理を持っていたことが原因で、コードレビューで「不正な`sortBy`だとデフォルト方向が反転/no-opになる」「JS側キャッシュ層に`host`/`kind`ソート時のtie-breakが無くSQL側と順序が食い違う」という2件の実バグが見つかった。`resolveErrorKindsSort`は`sortBy`を`['host','kind','count']`にclampした**後**に`sortOrder`のデフォルト（`count`のみ`desc`、それ以外`asc`）を決めることでこれを解消し、`error-kinds-cache.ts`の`sortErrorKindEntries`は`sortArrayItems`を3回（kind asc→host asc→本命の`sortBy`/`sortOrder`）安定ソートすることでSQL側と同じ`host`/`kind` ascのtie-breakを再現している。
>
> **tie-break順が未規定なのはlegacy(`getErrorKinds`)とread model間のみ**: `getErrorKinds`のitems配列は`count`降順にソートするが、同countの場合の並び順はJSの`Map`挿入順（`page_errors`→`crawl_errors`の処理順に依存する偶発的な順序）に委ねられている。read model系（`getViewerErrorKinds`とその出力を包む`error-kinds-cache.ts`）はどちらも`host`/`kind` ascのtie-breakで揃っており、read model系2つの間でのtie-break不一致は無い——不一致が残るのはlegacy経路の挿入順とread model系の`host`/`kind` asc順の間だけで、`getErrorKindsFastPath`が read model の staleness に応じて2経路を透過的に切り替えるため、データが変わらなくても切替タイミングだけで同点entryの表示順が変わりうる（表示順に依存する運用が必要な場合は呼び出し側で追加のソートキーを適用すること）。テスト（`get-viewer-error-kinds.spec.ts`/`build-viewer-read-model.spec.ts`）は比較前に`host`+`kind`でソートして正規化している。
>
> **dispatchは`getSummaryFastPath`と同型、キャッシュは「options非依存の全件スナップショット」だけを持つ**: `getErrorKindsFastPath`（`isViewerReadModelCurrent`ガード + `getViewerErrorKinds`/`getErrorKinds`の二択、`options`をそのまま透過）に集約し、viewer route（`register-error-kinds-route.ts`）はさらに`error-kinds-cache.ts`を経由する。`summary-cache.ts`と異なり`/api/error-kinds`は`host`/`kind`/`sortBy`/`sortOrder`/`limit`/`offset`というリクエストパラメータを持つため、素朴に「結果をarchiveId単位でキャッシュ」すると別のクエリパラメータの組み合わせに対して誤ったキャッシュ結果を返してしまう。そこでキャッシュ層は`getErrorKindsFastPath(accessor, {})`（オプション無し＝全件）の結果だけをarchiveId単位でLRU/ディスクキャッシュし、実際のリクエストの`host`/`kind`/`sortBy`/`sortOrder`/`limit`/`offset`は`applyErrorKindsOptions`（純粋関数、`getErrorKinds`と同じフィルタ/ソート/ページネーション契約をJSで再現）でリクエストごとに適用する。これにより異なるクエリパラメータの組み合わせでも、legacy fallback時の高コストな分類走査（後述ベンチマークで約542ms）は1回のみで済む。**stub modeは`getErrorKindsFastPath`を使わず常に`getErrorKinds`を実際の`options`付きで直接呼ぶ**（キャッシュも`applyErrorKindsOptions`も経由しない）——`summary-cache.ts`と同じ理由（`crawl --resume`/`--append`/`--retry-failed`が再オープンするtmpDirが既にread modelを持つ場合、`isViewerReadModelCurrent`のスキーマバージョン一致判定だけでは再開前の古いスナップショットを誤って返しかねない）。スキーマ変更のため`VIEWER_READ_MODEL_SCHEMA_VERSION`を8→9にbump（旧`groups/hosts/samples/meta`4テーブル時代の7→8を経て、dev側の破壊的変更への追従でさらにreshape）。
>
> **ベンチマーク実測**（`scripts/bench-viewer-error-kinds-read-model.mjs`、synthetic archive、実顧客データ不使用）: `crawl_errors`40万行規模（DNS/timeout/connection-refused/tlsの4kindに偏りを付けた合成分布、50ホスト、host×kindペアは高々200件程度に収束）で、read model build time 約552ms。直接関数呼び出し比較で`getErrorKinds`（legacy）約542ms → `getViewerErrorKinds`（read model）約1.6ms（約340倍）。`EXPLAIN QUERY PLAN`はフィルタ無し/`kind`フィルタとも`SCAN viewer_error_kind_entries USING INDEX vee_count`、`host`+`kind`同時指定（detail pane用）は`SEARCH ... USING PRIMARY KEY`。HTTP経由の`/api/error-kinds`はcold約602ms→約4.8ms、warm p50はキャッシュ層により両経路ともsub-ms（read modelの純粋な寄与は上記の直接呼び出し比較値で見るのが正しい、`summary-cache.ts`と同じ注意点）。

> **設計注意（`/api/resources`・`/api/unused-resources` の viewer_resources/viewer_resource_stats read model、issue #110）:** `listResources`（`resources`への行ごとcorrelated subqueryで`referrerCount`を計算）と`listUnusedResources`（`resources`→`resources-referrers`へのrequest-time anti-join、しかもCOUNT用とデータ取得用に2回実行）はGETのたびに再計算していた。`buildViewerReadModel`内で`resources`+`resources-referrers`を`resources.id`のkeysetでchunk化してスキャンする`computeResourceInsertRows`（`computeAnchorFactRows`と同じ「chunk化したスキャンから複数read modelテーブルの行を導出する」パターン、ただし`computeAnchorFactRows`はGROUP BYが複合キーのためid範囲パーティショニング、こちらは`GROUP BY resources.id`が主キーそのものなので単純なkeysetページネーションで済む——詳細は各ファイルのJSDoc）を追加し、`viewer_resources`（`resource_id`単位のfilter/sort列: `is_external`/`status`/`status_sort_key`/`status_desc_key`/`source`/`is_unused`/`url_sort_key`）と`viewer_resource_stats`（`resource_id`単位の`referrer_count`）に事前計算するよう変更した。**テーブルを2分割したのはissue本文のTO-BEが`viewer_resources`/`viewer_resource_stats`という2つのテーブル名を明示していたため**——機能的には`referrer_count`を`viewer_resources`へのカラム追加で済ませても支障は無いが、issueの命名との対応関係を優先した。`is_unused`は`viewer_resources`側にも複製している（`/api/unused-resources`のid解決フェーズがlimit前にこの列でフィルタする必要があるため、`viewer_anchor_facts.is_broken`と同じ設計）。**`content_category`列は意図的に持たない**——`viewer_pages`は`content_category`を持つが、`ListViewerResourcesOptions`/`ListViewerUnusedResourcesOptions`のどちらにもcontent-type-category相当のフィルタが無い（`ListResourcesOptions.contentType`は分類前の生MIME prefix文字列で別物、fast pathはこのフィルタが来ると legacy にfallbackする）。`viewer_pages`の列構成をそのままコピーして`content_category`を含めていた初期実装を、後述のベンチマークで参照者ゼロの死んだ列だと気付いて削除した。
>
> **cursorページネーションは`viewer_pages`型ではなく`viewer_anchor_facts`→`listViewerBrokenLinks`型を踏襲**: `/api/resources`と`/api/unused-resources`はどちらも同じ`viewer_resources`テーブルを読むが、固定ベース述語が異なる（前者はフィルタ無し、後者は`is_unused = 1`固定）。`viewer_pages`のような「1テーブル1cursorモジュールが全filter組み合わせを吸収する」設計ではなく、`viewer_anchor_facts`の`is_broken=1`固定subsetを読む`listViewerBrokenLinks`が独自のcursorスキームを持つのに倣い、`viewer-resources-cursor/`（`/api/resources`用）と`viewer-unused-resources-cursor/`（`/api/unused-resources`用）を完全に独立したモジュールとして実装した（encode/decode/sort-spec/filter-key の4ファイル×2セット、コード重複はあるが2つの固定ベース述語間でcursorが誤って混用されるリスクを型レベルで無くす）。fast pathが対応する`sortBy`は`/api/resources`が`url`/`status`のみ、`/api/unused-resources`が`url`/`status`/`source`のみで、それ以外（`statusText`/`contentType`/`isExternal`/`referrerCount`/`compress`/`cdn`/`contentLength`）や`urlPattern`/`contentType`フィルタ指定時はviewer route側（`register-resources-route.ts`/`register-unused-resources-route.ts`）が`registerPagesRoute`の`usesWideTableOnlyFilter`と同型の判定でlegacyに強制fallbackする。
>
> **`/api/resources/referrers`はread model非依存で常時bounded化**: `resources-referrers`テーブルは元から`unique(resourceId, pageId)`複合indexを持っており、`WHERE resourceId=? AND pageId>? ORDER BY pageId LIMIT ?`のcursor読み取りも`total`用の`COUNT(*) WHERE resourceId=?`もこの1本のindexで常にindex-coveredなため、専用read modelテーブルを作る必要が無いと判明した。`get-resource-referrers.ts`は`isViewerReadModelCurrent`分岐を持たず、どのarchiveに対しても直接bound/cursor化されたクエリを実行する。シグネチャを`(accessor, resourceUrl)`から`(accessor, {resourceUrl, limit?, cursor?})`に変更（v0.x のため破壊的変更として許容、3パラメータ以上はオブジェクトにまとめる規約にも合致）し、MCP `get_resource_referrers`・CLI `query resource-referrers --limit --cursor`を含む全3呼び出し元を更新した。
>
> **index設計はベンチマークで実際に破綻が見つかってから確定した**: `is_external`を先頭列に持つ複合index（`vr_default`/`vr_status`/`vr_status_desc`）だけを用意した初版を40万件の合成データで計測したところ、`isExternal`未指定のデフォルト表示（実運用で最も多いはずの呼び出し形）が`EXPLAIN QUERY PLAN`で`SCAN viewer_resources … | USE TEMP B-TREE FOR ORDER BY`に落ち、`/api/resources`のfast pathがlegacy（26-34ms）より遅い29-30msという**退行**を起こしていた。原因は`viewer_pages`が常に`content_category IN ('html','unknown')`という暗黙デフォルトフィルタを持つおかげでこの種の問題を回避できているのに対し、resourcesの`isExternal`はデフォルト値の無い純粋optionalフィルタで、リーディング列が未拘束のままだと複合indexが`ORDER BY`を満たせないため。`is_external`を先頭に置かない`vr_url_order`/`vr_status_order`/`vr_status_desc_order`を追加し、plannerに「`isExternal`指定時は`vr_default`等でSEARCH、未指定時は`vr_*_order`でSCAN」の両選択肢を持たせることで解消した（10 index構成、`viewer_pages`の8 indexと同程度の密度）。PR #96/issue #106の「先に実測、必要になってから足す」原則をここでも踏襲——最初から憶測でindexを積み増すのではなく、実測で退行を検出してから対応した点が重要。
>
> **ベンチマーク実測**（`scripts/bench-viewer-resources-read-model.mjs`、synthetic archive、実顧客データ不使用）: resources 40万行規模（内訳: 参照ありinternal 60%・unreferenced internal 25%（"unused"候補）・external 15%、referrer edgeは`pages`プールへの実FK付き）で、read model build time 約8.2秒、追加DBサイズ約260 MiB。直接関数呼び出し比較で`listResources`（legacy）約34ms → `listViewerResources`（read model）約6ms（約5.5倍）、`listUnusedResources`（legacy）約77ms → `listViewerUnusedResources`（read model）約4ms（約20倍）。HTTP経由は`/api/resources`が cold 約13ms→約6ms、`/api/unused-resources`が cold 約57ms→約4ms。`EXPLAIN QUERY PLAN`は`viewer_resources`のデフォルト表示が`SCAN … USING COVERING INDEX vr_url_order`（index修正後、TEMP B-TREE解消）、`is_unused=1`が`SEARCH … USING COVERING INDEX vr_unused`、`resources-referrers`が`SEARCH … USING COVERING INDEX`（既存の`unique(resourceId, pageId)`制約由来のindex、read model追加前後で変化なし、本来から高速）。

> **設計注意（`buildViewerReadModel`の大規模アーカイブOOM修正 + インデックス後付け化）:** `computeAnchorFactRows`/`computeResourceInsertRows`は元々`anchors`/`resources`全件を1回のSELECTでJS配列に展開してから`viewer_anchor_facts`/`viewer_resources`へchunk insertしていたが、実11GB級（pages 486,386件・resources 1,092,308件）アーカイブで検証したところ、この全件展開でJSヒープ上限（既定約4GB）に達しクラッシュすることを確認した（PR #168がURL自然順ソート用の一時テーブルで直していたのと同種の「テーブル全件を無制限にメモリ展開する」アンチパターン）。対処として両関数を`AsyncGenerator`化しchunk単位で読み込むよう変更——`computeResourceInsertRows`は`GROUP BY resources.id`が主キーそのものなので`read-url-chunks.ts`と同型の単純なkeysetページネーション（`WHERE id > lastId ORDER BY id LIMIT n`）、`computeAnchorFactRows`は`GROUP BY (source.id, destId)`が複合キーで1つの`source.id`が複数出力行を持ちうるため、集約後の出力をLIMITでページネーションすると1つの`source.id`のグループが途中で分割され得る（次chunkの`WHERE source.id > lastId`が残りを黙って読み飛ばす）——代わりに集約**前**の入力を`source.id`の非重複な範囲でパーティショニングする方式にした。`viewer_external_links`のchunk跨ぎ`referrer_count`合算は、JS側にMapを持ち回る初期実装がdistinct外部宛先の多いアーカイブでそれ自体無制限にメモリを消費しO(chunks×destinations)に劣化する欠陥がありxhighコードレビューで検出、`upsert-external-link-rows.ts`の`INSERT ... ON CONFLICT (dest_page_id) DO UPDATE SET referrer_count = referrer_count + excluded.referrer_count`でSQLite側に合算を委譲する設計に修正した。**この過程で判明したより大きな問題**: chunk化後もanchor_factsフェーズが実11GBアーカイブで29分経過しても完走しなかった。原因調査（`EXPLAIN QUERY PLAN`、chunkごとの区間タイミング計測、インデックス有無での挿入コスト比較）の結果、真因はchunk化そのものではなく、`create-viewer-read-model-tables.ts`が全14テーブルの索引を**データ投入前に**作成しており、`viewer_anchor_facts`（6 index）のような索引本数の多いテーブルへ数百万行規模で挿入する際、B-tree維持コストがテーブル成長とともに劣化する（実測: 1行あたり14.6µs→24.1µs→31.4µsと増加）ことだと特定した。索引を先に貼らず全データ投入後にまとめて構築する方式に変更したところ、同一データ量（100万行合成データ）で挿入+索引構築の合計が定数的な7.3µs/行に収まり、実11GBアーカイブでの推定所要時間が29分超（未完走）から2分未満に短縮された（SQLiteは索引を1回のソート済みスキャンで構築でき、行ごとのB-tree挿入+リバランスの累積より大幅に安い）。このため`createViewerReadModelTables`（テーブルのみ、索引なし）と`createViewerReadModelIndexes`（索引のみ、全テーブルのデータ投入完了後に`buildViewerReadModel`終盤で1回呼ぶ）に分割した——`viewer_anchor_facts`に限らず#103epicの全read modelテーブルに同じ理由で適用される。主キー・UNIQUE制約・`WITHOUT ROWID`はテーブル定義に残したまま（`.onConflict()`が使う一意性制約や重複拒否テストはこれらに依存し、後付けの対象は非必須の二次索引のみ）。ベンチマーク手法・数値は実データの匿名化された要約であり、実archiveのファイル名・ドメイン名はコード・コミット・ドキュメントのどこにも記録していない。

> **設計注意（`/api/images` の viewer_images read model、issue #113）:** `listImages`（`images` JOIN `pages`、`images`はこのコードベース最大の write-model テーブルで実archive実測3.25GB/idx_images_covering込み1.23GB）はGETのたびに再スキャンしていた。`buildViewerReadModel`内で`images`をkeysetでchunk化してスキャンする`computeImageInsertRows`（`computeResourceInsertRows`と同じ「`GROUP BY`無し・主キーそのものでchunk化」パターン）を追加し、`viewer_images`（`image_id`単位のfilter/sort列のみ）に事前計算するよう変更した。**単一テーブルのみ**——`viewer_resources`/`viewer_resource_stats`の2分割はissue #110本文のTO-BE命名を踏襲しただけで機能的必然性が無く、#113本文には2テーブル目の言及が無いため単一`viewer_images`に留めた。`src`/`currentSrc`/`alt`/`sourceCode`はいずれも大きい文字列列のため一切複製せず、`joinViewerImageIdsToListItems`でlimit後のid集合にのみ`images`/`pages`をjoinして解決する。
>
> **`page_url_rank`（整数サロゲート）はこのread modelファミリー内で唯一`url_sort_key`テキストをインライン複製しない設計**: `viewer_pages`/`viewer_resources`/`viewer_anchor_facts`/`viewer_directory_pages`はすべてURL由来の`url_sort_key`テキストをそのまま複製する統一パターンを取るが、`docs/viewer-db-redesign-plan.md`は`viewer_images`に限って「`page_url`を複製するな、911万行規模で危険」と名指しで警告している（他テーブルは高々ページ/リソース/エッジ単位で数十万行規模だが、imagesは1ページに複数画像が紐づくため実測で最大の行数になる）。`buildPageUrlRankMap`（`viewer-read-model/build-page-url-rank-map.ts`）が`viewer_pages`用に読み込み済みの`sourceRows`を再利用し、`viewer_pages.url_sort_key`と同じ順序で採番した密な0始まり整数ランクをbuild時に一度だけ計算してimage行へ複製する（4バイト/行、テキスト複製の60-150+バイト/行から大幅削減）。**ランクの比較はSQLiteのBINARY照合（UTF-8バイト列比較）に合わせてある**——素朴なJSの`<`/`>`文字列比較はUTF-16コード単位比較のため、補助面文字（U+10000以上、サロゲートペア）を含むURLで順序が食い違いうる（xhighレビューで検出、`compareUrlBinary`が`Buffer.compare(Buffer.from(a,'utf8'), Buffer.from(b,'utf8'))`で解決）。`viewer_images`テーブル自体には`page_id`列を持たない——`page_url_rank`の計算にのみ使われ計算後は一切読まれないため、911万行規模で未使用の整数列を持ち続けるのはこのテーブル自身が避けようとしている重複そのものになる。
>
> **`oversizedThreshold`は実行時の生値評価、専用indexなし**: `docs/viewer-sql-query-plan.md`の`oversized_1000`スケッチは固定閾値のbool事前計算だが、`listImages`の`oversizedThreshold`は呼び出し時に任意のpx数を受け取れる契約のため、fast pathでも`natural_width`/`natural_height`の生値を保持し`WHERE natural_width > ? OR natural_height > ?`で都度評価する（この1フィルタだけは大幅高速化しないが、`images`本体+`idx_images_covering`のスキャンよりは軽い）。
>
> **`missingAlt`/`missingDimensions`はtri-state（`true`/`false`/`undefined`）**——legacy `listImages`は元々truthyのみ判定（`false`=省略と同じ扱い）だったが、`listResources.isExternal`が確立している`!= null`規約に合わせてfast path（`applyViewerImagesFilters`）を実装したところ、同一の`{missingAlt: false}`が read model の鮮度という呼び出し元が制御できない内部状態によって legacy 経由では「全件」・fast path 経由では「alt有りのみ」と異なる結果を返す不整合をxhighレビューで検出した。正しい修正はfast pathを緩めることではなく、legacy側を`isExternal`と同じtri-state判定に揃えること（`list-images.ts`）——両経路が常に同じ入力に同じ結果を返す。
>
> **CLI/MCP/viewer routeを`getImagesFastPath`で統一**: `getErrorKindsFastPath`と同型の`isViewerReadModelCurrent`ガード+二択ディスパッチを`@nitpicker/query`側の共有ヘルパーとして新設し、CLI `query images`・MCP `list_images`・viewer `register-images-route.ts`の3呼び出し元すべてがこれを経由する。同じepicの`/api/resources`（issue #110）はroute層にしかfast path dispatchが無くCLI/MCPは今もlegacy直呼びのままという既存ギャップが残っているが、#113のスコープはimagesに限定し、そちらの是正は別issue対象とする。`cursor`/`direction`もroute層のクエリパラメータから素通しし、`viewer_images`用keysetカーソル機構がテスト以外の経路からも到達可能になっている（`register-resources-route.ts`と同じ理由でlegacy側にcursorシムは実装していない——legacy分岐は`nextCursor`/`prevCursor`とも常に`null`、フロントの`use-images-infinite.ts`はoffsetのみを使うため実害なし）。
>
> **ベンチマーク実測**（`scripts/bench-viewer-images-read-model.mjs`、synthetic archive、実顧客データ不使用）: images 40万行規模（1ページあたり8画像、missing alt 10%・missing dimensions 5%・oversized 2%の合成分布）で、read model build time 約5.8秒、追加DBサイズ約83.5 MiB（read model全14→15テーブル合計、`viewer_images`単体の寄与ではない）。直接関数呼び出し比較で`listImages`（legacy）約17.9ms→`listViewerImages`（read model）約2.2ms（約8倍）。HTTP経由`/api/images`はcold約20.2ms→約3.8ms。`EXPLAIN QUERY PLAN`はデフォルト表示が`SCAN viewer_images USING COVERING INDEX vi_default`、`missing_alt=1`が`SEARCH viewer_images USING COVERING INDEX vi_missing_alt`。
>
> **共通化リファクタ（`viewer-cursor-kit/`）**: xhighレビューで「`list-viewer-images.ts`が`list-viewer-resources.ts`とほぼ同一のkeyset pagination実装を重複している」と指摘され、ユーザーの明示的な許可を得て#113のスコープを拡張し、pages/resources/unused-resources/anchor-facts（broken-links）/imagesの計5モジュールが共有する`packages/@nitpicker/query/src/viewer-cursor-kit/`を新設した。`applyKeysetPredicate`（`(col1,col2,…) {>|<} (?,?,…)`のrow-value比較WHERE句生成）・`readKeysetWindow`（filter適用→keyset predicate→ORDER BY→`limit+1`の機械的な窓読み取り、テーブル名・filter関数・select列を引数化）・`extractKeysetSortValues`・`buildFilterKey`（`undefined→null`正規化+`JSON.stringify`、呼び出し元が渡すオブジェクトリテラルのキー順をそのまま使うため各テーブルの既存filterKey文字列と互換）・`encodeCursorEnvelope`/`decodeCursorEnvelope`（`{v,filterKey,sortBy,sortOrder,values}`envelopeの共通処理、anchor-facts専用の位置別型チェックは`expectedValueTypeAt`コールバックとして注入できる拡張点を用意）の6関数に集約した。各テーブル固有の`types.ts`（SortColumn union）・`get-*-sort-spec.ts`（テーブル固有の`_desc_key`負値化ロジック）・オーケストレータの「id解決後にwideテーブルをjoinする/しない」判断はテーブルごとに残す——完全な1オーケストレータ関数への統合はpages（facets付加）・unused-resources/anchor-facts/directory-pages（`offset`/`limit`/`total`をレスポンスに含めない）の構造差が大きく、過度な抽象化になると判断した。**`directory-pages-cursor`は対象外**——`sortBy`/`filterKey`/`direction`/`offset`/`total`のいずれも持たない別設計（固定1ソート・forward-onlyの`nodeId`スコープ専用カーソル）で、この重複パターンには該当しない。移行は各テーブルを1つずつ移行してテストを実行する形で行い、既存33 specファイルのアサーションは全て無変更のまま通過を確認した（filterKeyのJSON文字列・エラーメッセージ文言・anchor-factsの位置別型検証を含め、可観測な振る舞いの変更なし）。詳細は各ファイルのJSDoc（特に`read-keyset-window.ts`のパラメータ説明）を正とする。

> **設計注意（`/api/headers` の viewer_header_checks read model、issue #119）:** `checkHeaders`は`pages.responseHeaders`（生JSON文字列）に対する`headerPresenceExpression`のLIKEスキャン（JSON parseはしないがフルスキャンは毎回発生）+offset-onlyページネーションのみで、`viewer_*`系read modelもcursor pagination対応も持たない孤立した実装だった。事前調査で判明した2点を踏まえてスコープを確定した: (1) viewer に`/api/headers`ルート自体が存在せず、CSP等のヘッダーチェックは`/api/pages`のフィルタ（`hasCSP`等）として統合されている。(2) `docs/viewer-sql-query-plan.md`の`/api/headers`SQL例は`has_referrer_policy`/`has_permissions_policy`/`cache_policy`という現行コードに存在しない列と、`header_sets`/`content_items`/`url_refs`という未実装のref-tableモデル（issue #139範疇、#114で当時見送られた将来像）を前提にしており、#119のDon't（"Do not mix this issue with full write model migration"）に反するため採用しない。**採用したスコープ**: (a) CLI `query headers`・MCP `check_headers`・新設viewer route `/api/headers`の3経路を`getHeaderChecksFastPath`（`getImagesFastPath`と同型のディスパッチヘルパー）に統一し、`viewer_header_checks`読み取りにfast path化、(b) 既存の`/api/pages`のヘッダーフィルタ時legacy fallback（`register-pages-route.ts`のJSDoc記載の既知の意図的な制約）はスコープ外・現状維持とした——`viewer_pages`は`is_external`/`content_category`等のfilter/sort列のみを持つ設計であり、header presenceのようなread-model横断の複合filterを`viewer_pages`にJOINで持ち込むのは#119単体のスコープを大きく超える。`viewer_header_checks`（`page_id`主キー、`url_sort_key`/`has_csp`/`has_x_frame_options`/`has_x_content_type_options`/`has_hsts`/`missing_count`）は`checkHeaders`と全く同じ`scraped=1, isExternal=0, contentType='text/html', redirectDestId IS NULL`述語で`pages`から1回SQLを発行し（`viewer_pages`の`sourceRows`は`excludeSkippedPages`込み・content-type無制限のため述語が異なり再利用不可)、`buildHeaderPresenceSelects`が生成する既存のLIKE式で4フラグをSQL側のみで計算する（`responseHeaders`列自体は一切SELECTしない）。`url_sort_key`は`viewer_pages.url_sort_key`と同じ「`url`を verbatim 複製」規約で、ページ数と同オーダーの header-check table では #139 後もこの inline sort key を維持する。`has_csp`等の個別フラグには専用索引を作らない（#106/#118のevidence-before-indexing原則、`getHeaderChecksFastPath`は明示的な`sortBy`が指定されれば常にlegacyへ強制フォールバックするため個別フラグsortを高速化する必要がない）。**`vh_missing`は`docs/viewer-sql-query-plan.md`記載の`(missing_count, url_sort_key, page_id)`をそのまま採用せず、`(is_missing, url_sort_key, page_id)`という別列で構成**——xhighレビューで、`missing_count > 0`という範囲述語を先頭列に持つ索引は`ORDER BY url_sort_key`を満たせない（SQLiteは`missing_count`の値ごとにurl順の区間を順に舐めるだけで、全体としてurl順にはならない）ため`EXPLAIN QUERY PLAN`で実際には常に`vh_default`にフォールバックし、`missingOnly`フィルタが意図した高速化を提供しないデッドウェイトになっていることが実測で確認された。`viewer_pages`が`has_title`等の**カウントではなくbool値**を索引先頭列に置く既存の設計判断と同じ理由（等価述語なら先頭列で絞り込んだ後もその値の範囲内で次列の順序が保たれる）で、`missing_count`（0-4の生の集計値、detail表示用に保持）とは別に`is_missing`（`missing_count > 0`のbool、フィルタ専用）を追加した。回帰防止のため`list-viewer-header-checks.spec.ts`に`EXPLAIN QUERY PLAN`テスト（500行の合成データで`vh_missing`使用かつ`TEMP B-TREE`不使用を検証）を追加した。**もう1点、xhighレビューで検出した正当性バグ**: `getHeaderChecksFastPath`の初期実装は`sortBy: 'url'`を明示指定した場合も「未指定」と同一視してfast pathへルーティングしていたが、legacy `checkHeaders`は明示的な`sortBy: 'url'`を「自然順（数値対応）ソート」の要求として扱う（`applyListOrder`の`type: 'url'`分岐、`useUrlSort = options.sortBy != null`）のに対し、`listViewerHeaderChecks`は常に`url_sort_key`列への単純なSQLite `BINARY`照合`ORDER BY`（`checkHeaders`の"`sortBy`未指定"のplain分岐と等価）しか提供しない。両者は異なるソートアルゴリズムであり、fast pathへの誤ルーティングはread modelの鮮度次第でソート順がサイレントに変わる不具合になるため、`usesWideTableOnlyFilter`の判定条件を「`sortBy`が非nullなら常にlegacyへ」に修正した（`'url'`を除外しない）。`VIEWER_READ_MODEL_SCHEMA_VERSION`を11→12にbump。

> **設計注意（viewer diagnostic tables、issue #115）:** `findDuplicates`（`GROUP_CONCAT(url, X'1F') GROUP BY <field>`一発、しかしメンバー全件を1つのテキスト列に詰めるためoffsetページネーションが原理的に成立しない）と`findMismatches`（`pages`への都度WHERE、offset-onlyページネーション）を、それぞれ`viewer_duplicate_groups`/`viewer_duplicate_group_pages`（group-level/member-page tableの2分割）と`viewer_mismatches`（1ページ1行）に置き換えた。**なぜ2テーブルに分割したか**: `viewer_resources`/`viewer_resource_stats`・`viewer_directory_nodes`/`viewer_directory_pages`と同じ「grainごとに1テーブル」パターンを踏襲——duplicate groupには「グループ自体の集計値（field/value/count）」と「メンバーページの完全一覧」という2つの異なるgrainがあり、後者は数千件規模になり得るため、前者（`/api/duplicates`のgroup一覧）に無制限にJOINで埋め込むのではなく、`/api/duplicates/:groupId/pages`という別カーソルページネーションエンドポイントに逃がした。`viewer_mismatches`は元々1ページ1行のgrainしか無いため分割不要。
>
> **ビルドは2フェーズ**: `computeDuplicateGroupRows`が`title`→`description`の順に`GROUP BY <field> HAVING COUNT(*) > 1`を1回ずつ実行し、各グループへ`group_id`をJS側で連番採番して`groupIdByValue`（`Map<field, Map<value, groupId>>`）を返す。続く`computeDuplicateGroupPageRows`が`pages`を`id`keysetでchunk scanしながら各行の`title`/`description`を`groupIdByValue`で引き当て、該当すれば`viewer_duplicate_group_pages`行を生成する。`group_id`をSQLite側`AUTOINCREMENT`に任せず先にJS側で確定させる理由は`viewer_directory_nodes.node_id`と同じ——`viewer_duplicate_group_pages`行が挿入前から`group_id`を参照する必要があるため。対照的に`viewer_mismatches.mismatch_id`は`viewer_anchor_facts.edge_id`と同じくSQLite側`AUTOINCREMENT`任せ（他のどのテーブルも挿入前にmismatch行のidを参照する必要が無いため）。`computeMismatchInsertRows`は`canonical`→`og:title`→`og:description`の順に`pages`を`id`keysetでchunk scanする`AsyncGenerator`（`buildViewerReadModel`の大規模アーカイブOOM修正で確立した「chunk化してyield」パターンを最初から適用、この機能に旧・非chunk化版は存在しない）。
>
> **inlineページサンプルのN+1回避**: `/api/duplicates`のgroup一覧は各グループの先頭`pagesLimit`件（既定20件）をinlineで返す。これを`fetchDuplicateGroupPageSamples`が`SELECT group_id, url_sort_key FROM (SELECT ..., ROW_NUMBER() OVER (PARTITION BY group_id ORDER BY url_sort_key, page_id) AS rn FROM viewer_duplicate_group_pages WHERE group_id IN (...)) WHERE rn <= ?`という単一のwindow関数クエリで、1ページ分（既定100グループ）のサンプルをまとめて取得する。単純な`WHERE group_id IN (...) ORDER BY group_id, url_sort_key LIMIT groupIds.length * pagesLimit`ではダメな理由: メンバー数が偏っている場合、大きいグループの行が`LIMIT`予算を独占し、後続グループがサンプルを1件も得られない可能性がある。`PARTITION BY group_id`によって各グループが独立に`pagesLimit`件までを保証される。
>
> **索引**: `vdg_field_count (field, count_desc_key, group_id)`と`vm_type_url (type, url_sort_key, mismatch_id)`——`field`/`type`という必須の等価フィルタを先頭に置き、続く`_desc_key`/`url_sort_key`でソート順を索引順そのままにする"equality-then-sort-key"の形は`vh_default`と同型（#106/#118のevidence-before-indexingを踏襲）。`viewer_duplicate_group_pages`は当初「`(group_id, page_id)`複合`PRIMARY KEY`のWITHOUT ROWIDテーブルなので専用indexは不要」と判断していたが、xhighレビューで`EXPLAIN QUERY PLAN`実測により誤りと判明した：このPKは`page_id`順にクラスタリングするだけで、実際の読み取り順である`(url_sort_key, page_id)`は満たさず、`SEARCH ... USING PRIMARY KEY (group_id=?)`の後に必ず`USE TEMP B-TREE FOR ORDER BY`が発生する（同一テンプレートのボイラープレートtitleが数千ページに及ぶような大規模グループでは毎回全メンバーを再ソートすることになる）。`vdg_field_count`/`vm_type_url`と同じ"equality-then-sort-key"の形で`vdgp_group_url (group_id, url_sort_key, page_id)`を追加し解消した。
>
> **fast path dispatchの非対称性**: `getDuplicatesFastPath`は`findDuplicates`に`sortBy`/`urlPattern`の概念自体が無いため、read modelが有効な限り常にfast pathを優先する——`getHeaderChecksFastPath`/`getMismatchesFastPath`のような「特定条件で強制legacy」分岐を持たない、このread modelファミリー内で唯一のdispatchヘルパーになっている。一方`getMismatchesFastPath`は`options.sortBy != null || options.urlPattern != null`のいずれかで常にlegacyへフォールバックする——`viewer_mismatches`は`(type, url_sort_key, mismatch_id)`しか索引しておらず、`actual`/`expected`列でのソートや`urlPattern`（sourceとdestの両方に一致し得るLIKE検索）を支えられないため。
>
> **`findMismatches`の潜在バグを発見・修正**: この#115のroute配線で初めて`/api/mismatches?type=canonical&sortBy=...`という組み合わせがテストされ、`no such table: viewer_url_sort_keys`でクラッシュする既存バグが判明した。原因は`findMismatches`の`canonical`タイプにおける列設定`expected: { column: '"pages"."canonical"', type: 'url' }`——`type: 'url'`が`useUrlSort`フラグと無関係に常に付いており、`applyListOrder`経由で`orderByUrlRank`（`viewer_url_sort_keys` TEMP TABLEへの`SELECT`を伴うサブクエリ）を要求する。ところが`findMismatches`は`listPages`/`listExternalLinks`と異なり一度も`ensureUrlSortTempTable`を呼んでいなかったため、TEMP TABLEが存在せずクラッシュしていた。分析の結果、`findMismatches`がこの経路に入るのは`useUrlSort`（＝`options.sortBy != null`、明示的な`sortBy`指定）が真の場合に限られる（`sortBy`未指定のデフォルトパスは常に`type: 'plain'`の列を選ぶため無関係）と判明したため、`useUrlSort`が真の時だけ`ensureUrlSortTempTable(accessor)`を呼ぶよう修正した。`listPages`のように「デフォルトのsortByが`'url'`なので常に呼ぶ」設計にしなかった理由は、`getMismatchesFastPath`のlegacy fallbackはread modelが単に古い/未構築という最頻ケースでも`options`オブジェクト形式で`findMismatches`を呼ぶため、そこで無条件にTEMP TABLE構築（外部マージソート）のコストを払うと、このread modelエピック全体が守ろうとしている「fallbackも軽量に保つ」という目標に反するため。`find-mismatches.spec.ts`に`sortBy: 'actual' | 'expected' | 'url'`それぞれの回帰テストを追加した。
>
> **CLI/MCPの応答形状変更**: CLI `query duplicates`/`query mismatches`・MCP `find_duplicates`/`find_mismatches`・viewer `/api/duplicates`/`/api/mismatches`の3経路を、それぞれ`getDuplicatesFastPath`/`getMismatchesFastPath`（`getHeaderChecksFastPath`/`getImagesFastPath`と同型の統一ディスパッチヘルパー）に一本化した。これに伴い、CLIの`findDuplicates`直接呼び出しが返していた素の配列、MCPの`findMismatches`位置引数オーバーロードが返していた素の配列は、いずれも`{items, total, limit, offset, nextCursor, prevCursor}`のcursor付きオブジェクトに変わった（v0.x方針により後方互換は取らない、破壊的変更として許容）。MCP `find_duplicates`/`find_mismatches`は既存の`field`/`type`個別バリデーションを維持しつつ、初期実装は`omit(args, 'archiveId', ...)`でその他の引数をfast pathへ生のまま渡していた——xhighレビューで、これが`check_headers`と同じく`limit`/`offset`等の型検証を失わせるバグだと判明し（数値の文字列が`Number("10")+1`ではなく`"10"+1`＝`"101"`に文字列連結されるkeyset窓計算の破損リスク）、両ツールとも既存の`optionalNumber`/`optionalString`ヘルパーで明示的に検証するよう修正した（`check_headers`自体は#115のスコープ外のため未修正のまま）。`find_duplicates`のtool定義（inputSchema）に`offset`/`pagesLimit`を追加し、`find_mismatches`のtool定義に`urlPattern`/`sortBy`/`sortOrder`を追加した——実装時に「`check_headers`がcursorを公開しない前例に倣い変更しない」と判断していたが、`offset`はcursorと異なりCLI/viewer routeの両方が既に一般的なページネーション手段として公開している値であり、MCPツールのスキーマから欠落しているとLLMクライアントがその存在を発見できず、`total`の実件数を得られても実際にページを進める手段がないという実害があったため方針を修正した。
>
> `/api/duplicates/:groupId/pages`はread modelが無い/古い場合404を返す。CLI `query duplicates`/`query mismatches`には`--cursor`/`--direction`/`--pagesLimit`フラグを追加した（`--cursor`は既存、`--direction`/`--pagesLimit`が新規）。`VIEWER_READ_MODEL_SCHEMA_VERSION`を12→13にbump。40万件の合成データ実測（`scripts/bench-viewer-duplicates-mismatches-read-model.mjs`、`example.com`のみ使用）で、直接呼び出し比較`findDuplicates`約273.9ms→`getDuplicatesFastPath`約5.8ms、`findMismatches`約68.9ms→`getMismatchesFastPath`約2.1ms、HTTP経由で`/api/duplicates`約195.7ms→約14.0ms（約14倍）、`/api/mismatches`約75.5ms→約3.2ms（約24倍）。
>
> **`getDuplicatesFastPath`のlegacy fallbackで検出・修正した3件の実バグ（xhighレビュー）**: 初期実装は(1) `total: items.length`——`findDuplicates`の`limit`で切り詰められた後の件数をそのまま`total`扱いしており、read modelが古い/未構築のアーカイブでは実際のグループ総数より小さい値がレスポンスに残り続け、`nextCursor`も常に`null`のため残りのグループへ到達する手段が無かった。`findDuplicates`と同じ`GROUP BY <field> HAVING COUNT(*) > 1`述語を`COUNT(*)`でラップするだけの新規`countDuplicateGroups`（`count-duplicate-groups.ts`、distinct重複値の件数に比例するコストで済む）を追加して修正。(2) `options.offset`を無視——`findDuplicates`にそもそも`offset`パラメータが無かったため、CLI/MCP/viewer routeがoffsetを渡してもlegacy分岐では常に先頭ページが返っていた。`findMismatches`に既にある`offset`対応に合わせ、`findDuplicates`にも第4引数`offset`（デフォルト0）を追加した。(3) `DEFAULT_LIMIT`（50）をlegacy分岐にしか適用しておらず、fast path分岐は`options.limit`を無検証で`listViewerDuplicateGroups`へ渡していた——同関数自身のデフォルト（100）が漏れて、`--limit`省略時の結果件数がread modelの鮮度だけで50↔100に変わっていた。`limit`の解決を関数冒頭1箇所に集約し、両分岐へ明示的に渡すよう修正。
>
> **legacy分岐の`groupId`は非正の整数sentinel（`-(index+1)`）**: 当初`index+1`という正の連番だったため、`viewer_duplicate_groups.group_id`（`INTEGER PRIMARY KEY`、1始まり）と衝突しうる別の実バグをxhighレビューで検出した——`/api/duplicates`をread model未構築時に呼び出し、直後に（`viewer-build`や自動ビルド完了により）read modelが有効になった状態で同じ数値を`/api/duplicates/:groupId/pages`に渡すと、無関係な実グループのメンバー一覧を誤って返す。legacy分岐は元々`findDuplicates`がメンバーURLを無制限に全件返す設計（truncationが無い）ため、そもそもドリルダウン先を持つ必要が無い——`pages`を`pagesLimit`で切り詰めるのもやめ、常に完全なメンバー一覧を返すよう修正した。`groupId`を実IDと衝突し得ない非正の値にし、`register-duplicates-route.ts`側で`groupId <= 0`を即404にするガードを追加して、read model構築タイミングに依存する誤動作を構造的に防いだ。
...
> **受容済みの遅い query**: `listLinks` 13-16s (anchor SCAN + 3-way JOIN + COALESCE、SQL-first push-down 不能、`canonicalId` 列の denormalisation 待ち), `computeIsolatedClusters` 17s (66k inventory pages + 5M anchors、SQL-side filter 試したが 11.6s で改善なし、`isolated_root` 列の denormalisation 待ち)。各 query.ts の JSDoc に「push-down 不能の根拠」と「次のステップ (schema 変更)」を記載。**`getSummary` の 22s (4 並列 COUNT) は issue #104 で `viewer_summary` read model に解消済み** — read model が current な場合は `getSummaryFastPath` 経由で `getViewerSummary`（単一行 SELECT）が答える、詳細は「設計注意（`/api/summary` の viewer_summary read model、issue #104）」を参照。
...
**referrer 単位への集約:** `getPageDetail.inboundLinks` は `anchors.pageId`（referrer）で `GROUP BY` し、1 referrer につき 1 行を返す。下記「なぜ tuple 重複削除 / `UNIQUE` 制約を使わないか」の通り、同一ページが header/footer 等で同じ宛先に複数アンカーを持つのは storage 層では正当な重複だが、被リンク一覧としては「何ページから参照されているか」が知りたいので referrer 単位に畳む。`listExternalLinks` の `referrerCount`（`COUNT(DISTINCT source.id)`）も同じ理由で同じ粒度に揃えてある — 両者が食い違うと外部リンク一覧の参照元数と Page Detail の被リンク件数が一致しなくなる。
