...
| サブコマンド         | 説明                                                                                                       | 必須オプション |
| -------------------- | ---------------------------------------------------------------------------------------------------------- | -------------- |
| `summary`            | サイト全体の概要（内部/外部のページ数とコンテンツ数、ステータス分布、Content-Type 分布、メタデータ充足率） | なし           |
...
| `--contentTypeCategory` | 文字列 | なし       | `pages` のみ。Content-Type カテゴリで絞り込み（`html`, `pdf`, `csv`, `word`, `excel`, `powerpoint`, `image`, `css`, `javascript`, `json`, `xml`, `font`, `audio`, `video`, `archive`, `text`, `other`, `unknown`）。`csv` は CSV+TSV、`word` は .doc+.docx、`excel` は .xls+.xlsx、`powerpoint` は .ppt+.pptx、`json` は JSON+YAML、`text` は .txt+.md を 1 カテゴリに統合する。指定時は既定の HTML-or-null ベースフィルタを外し、PDF など非 HTML 行も列挙する |
...
> **Content-Type 分類**: Summary 画面に 18 カテゴリ（`html`/`pdf`/`csv`/`word`/`excel`/`powerpoint`/`image`/`css`/`javascript`/`json`/`xml`/`font`/`audio`/`video`/`archive`/`text`/`other`/`unknown`）の **シェア（合計に対する内訳）を 1 本の積み上げバー + 凡例** で表示する（macOS / iOS のストレージ画面と同じ可視化）。凡例は 1 カラム縦並びで各行に拡張子をカッコ書きで併記し、0 件のカテゴリも「測定したが該当なし」として表示される。各セグメントは固定色 + 罫線/水玉/格子のパターンオーバーレイを常時持つので、色覚多様性のあるユーザでもカテゴリを区別できる。サブピクセルセグメントは CSS の `min-inline-size: 4px` でホバー可能な細片に底上げされる一方、JS は再正規化せず生のシェアを返すため凡例の % はバー幅と一致する。**カテゴリ統合**: CSV と TSV、.doc と .docx (Word)、.xls と .xlsx (Excel)、.ppt と .pptx (PowerPoint)、JSON と YAML、.txt と .md (Text) はそれぞれ 1 カテゴリにまとめている。Pages 画面のフィルタバーで Content-Type を選ぶと、既定の HTML-or-null 制約を外して PDF 等の非 HTML ページも一覧できる（例: 「30 万 URL の大半が PDF」を直接ブラウズ）。カテゴリ判定は `@nitpicker/query` の `classifyContentType` ルール表に集約され、Summary チャートと Pages フィルタが同じ行を同じカテゴリに数える。
...
| `get_summary`            | サイト全体の概要（内部/外部のページ数とコンテンツ数、ステータス分布、Content-Type 分布、メタデータ充足率） |
